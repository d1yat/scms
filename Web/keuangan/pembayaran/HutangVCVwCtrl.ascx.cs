...
public partial class keuangan_pembayaran_HutangVCVwCtrl : System.Web.UI.UserControl
{
  #region Private
  ...
  private void ClearEntrys()
  {
    ...
    txBiayaAdminHdr.Disabled = false;
    chkDPHdr.Checked = false;
    chkDPHdr.Disabled = false;
  ...
  private void PopulateDetail(string pName, string pID)
  {
    ...
    string res = soa.GlobalQueryService(0, 1, false, string.Empty, string.Empty, "0084", paramX);
    ...
    try
    {
      ...
      if (dicResult.ContainsKey("records") && (dicResult.ContainsKey("totalRows") && (((long)dicResult["totalRows"]) > 0)))
      {
        ...
        winDetail.Title = string.Format("Debit - {0}", pID);
        ...
        suplId = dicResultInfo.GetValueParser<string>("c_nosup", string.Empty);
        Functional.SetComboData(cbSuplierHdr, "c_nosup", dicResultInfo.GetValueParser<string>("v_nama_suplier", string.Empty), suplId);
        ...
        txBiayaAdminHdr.Text = dicResultInfo.GetValueParser<decimal>("n_admin").ToString();
        txBiayaAdminHdr.Disabled = true;

        chkDPHdr.Checked = dicResultInfo.GetValueParser<bool>("l_um");
        chkDPHdr.Disabled = true;
    ...
    catch (Exception ex)
    {
      System.Diagnostics.Debug.WriteLine(
        string.Concat("keuangan_pembayaran_HutangVCVwCtrl:PopulateDetail Header - ", ex.Message));
    }
    finally
    {
      if (jarr != null)
    ...
    {
      System.Diagnostics.Debug.WriteLine(
        string.Concat("keuangan_pembayaran_HutangVCVwCtrl:PopulateDetail Detail - ", ex.Message));
    }
  ...
  private PostDataParser.StructureResponse SaveParser(string numberID, Dictionary<string, string>[] dics)
  {
    ...
    pair.DicAttributeValues.Add("Suplier", cbSuplierHdr.SelectedItem.Value);
    ...
    pair.DicAttributeValues.Add("AdminTax", txBiayaAdminHdr.Text);
    pair.DicAttributeValues.Add("IsDownPayment", chkDPHdr.Checked.ToString().ToLower());
    ...
    {
      Scms.Web.Common.Logger.WriteLine("keuangan_pembayaran_HutangVCVwCtrl SaveParser : {0} ", ex.Message);
    }
