...
public partial class keuangan_faktur_FakturBeliProses : Scms.Web.Core.PageHandler
{
  #region Private
    ...
    try
    {
      ...
      if (dicResult.ContainsKey("records") && (dicResult.ContainsKey("totalRows") && (dicResult.GetValueParser<long>("totalRows") > 0)))
      {
        ...
        for (nLoop = 0, nLen = listDicResultInfo.Count; nLoop < nLen; nLoop++)
        {
          ...
          lst.Add(dicResultInfo.GetValueParser<string>("RnNo", string.Empty));
          lst.Add(dicResultInfo.GetValueParser<string>("DoNo", string.Empty));
          lst.Add(dicResultInfo.GetValueParser<string>("Supplier", string.Empty));
          lst.Add(dicResultInfo.GetValueParser<string>("Keterangan", string.Empty));
