...
public partial class keuangan_faktur_FakturBeliRetur : Scms.Web.Core.PageHandler
{
  private PostDataParser.StructureResponse DeleteParser(string numberId, string ket)
  {
    ...
    try
    {
      varData = parser.ParserData("FakturBeliRetur", "Delete", dic);
    }
    catch (Exception ex)
    {
      Scms.Web.Common.Logger.WriteLine("keuangan_faktur_FakturBeliRetur DeleteParser : {0} ", ex.Message);
    }
  ...
  public void DeleteMethod(string noteNumber, string keterangan)
  {
    ...
    if (respon.Response == PostDataParser.ResponseStatus.Success)
    {
      ...
      X.AddScript(
        string.Format("deleteFromStoreMultiple({0}, ['c_fbno'], ['{1}']);",
          storeGridMain.ClientID, noteNumber));
  ...
  protected void Page_Load(object sender, EventArgs e)
  {
    if (!this.IsPostBack)
    {
      FakturBeliReturCtrl1.Initialize(storeGridMain.ClientID);
    }
  }
  ...
  protected void gridMainCommand(object sender, DirectEventArgs e)
  {
    ...
    {
      FakturBeliReturCtrl1.CommandPopulate(false, pID);
    }
  ...
  //  FakturBeliCtrl1.CommandPopulate(true, null);
  //}
}
