...
public partial class transaksi_penerimaan_RNRetur : System.Web.UI.UserControl
{
  #region Private
  ...
  private void ClearEntrys()
  {
    ...
    winDetail.Title = "Receive Notes Retur";
  ...
  private void PopulateDetail(string pName, string pID, string gdgID)
  {
    ...
    try
    {
      ...
      if (dicResult.ContainsKey("records") && (dicResult.ContainsKey("totalRows") && (((long)dicResult["totalRows"]) > 0)))
      {
        ...
        winDetail.Title = string.Format("Receive Notes Retur - {0}", pID);
    ...
    catch (Exception ex)
    {
      System.Diagnostics.Debug.WriteLine(
        string.Concat("transaksi_penerimaan_RNReturCtrl:PopulateDetail Header - ", ex.Message));
    }
    finally
    {
      if (jarr != null)
    ...
    {
      System.Diagnostics.Debug.WriteLine(
        string.Concat("transaksi_penerimaan_RNReturCtrl:PopulateDetail Detail - ", ex.Message));
    }
  ...
  private PostDataParser.StructureResponse SaveParser(bool isAdd, string rnNumber, Dictionary<string, string>[] dics, string gudangID, string suplierID, string nomorDO, DateTime tglDO, float bea, string keterangan)
  {
    ...
    pair.DicAttributeValues.Add("TypeRN", "02");
    ...
    try
    {
      varData = parser.ParserData("RNRetur", (isAdd ? "Add" : "Modify"), dic);
    }
    catch (Exception ex)
    {
      Scms.Web.Common.Logger.WriteLine("transaksi_penerimaan_RNReturCtrl SaveParser : {0} ", ex.Message);
    }
  ...
  protected void SaveBtn_Click(object sender, DirectEventArgs e)
  {
    ...
    Dictionary<string, string>[] gridDataRS = JSON.Deserialize<Dictionary<string, string>[]>(jsonGridValues);

    PostDataParser.StructureResponse respon = SaveParser(isAdd, numberId, gridDataRS, strGdgID, strSuplID, strNumberDO, dateDO, fBea, strKeterangan);
