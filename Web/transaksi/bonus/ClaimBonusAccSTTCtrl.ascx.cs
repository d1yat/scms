...
public partial class transaksi_bonus_ClaimBonusAccSTTCtrl : System.Web.UI.UserControl
{
  ...
  private void PopulateDetail(string pName, string pID)
  {
    ...
    try
    {
      ...
      if (dicClaimAcc.ContainsKey("records") && (dicClaimAcc.ContainsKey("totalRows") && (((long)dicClaimAcc["totalRows"]) > 0)))
      {
        ...
        winDetail.Title = string.Format("Claim Acc STT List - {0}", pID);
      }
    }
    catch (Exception ex)
    {
      ...
      if (respon.Response == PostDataParser.ResponseStatus.Success)
      {
        ...
        if (isAdd)
        {
          if (respon.Values != null)
          {
            ...
            {
              dateJsPrins = Functional.DateToJson(date);
            }
            if (!string.IsNullOrEmpty(strStoreID))
            {
              ...
              scrpt = string.Format(@"{0}.insert(0, new Ext.data.Record({{
                ...
                'v_nama': '{6}',
                    ...
                    ClaimAccID,
                    dateJs, NoClaimAcc,
                    dateJsPrins, NoClaim, PrincipalDesc);
        ...
        PopulateDetail("c_claimaccno", numberId);
