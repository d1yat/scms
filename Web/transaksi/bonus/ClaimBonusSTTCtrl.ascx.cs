...
public partial class transaksi_bonus_ClaimBonusSTTCtrl : System.Web.UI.UserControl
{
  ...
  private void PopulateDetail(string pName, string pID)
  {
    ...
    string res = soa.GlobalQueryService(0, 1, false, string.Empty, string.Empty, "0062", paramX);
    ...
    try
    {
      ...
      if (dicClaim.ContainsKey("records") && (dicClaim.ContainsKey("totalRows") && (((long)dicClaim["totalRows"]) > 0)))
      {
        ...
        winDetail.Title = string.Format("Retur Claim STT List - {0}", pID);
      }
    }
    catch (Exception ex)
    {
