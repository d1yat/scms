...
public partial class transaksi_pengiriman_EkspedisiPrincipal : Scms.Web.Core.PageHandler
{
    #region Private

    private void ClearEntrys()
    {
        winDetail.Title = "Ekspedisi Principal";
        ...
        Ext.Net.Store cbRSDtlstr = cbRSDtl.GetStore();
        if (cbRSDtlstr != null)
        {
            cbRSDtlstr.RemoveAll();
        }
    ...
    private void PopulateDetail(string pName, string pID)
    {
        ...
        string res = soa.GlobalQueryService(0, 1, false, string.Empty, string.Empty, "0216", paramX);
        ...
        try
        {
            ...
            if (dicEXP.ContainsKey("records") && (dicEXP.ContainsKey("totalRows") && (((long)dicEXP["totalRows"]) > 0)))
            {
                ...
                Functional.SetComboData(cbPrincipalHdr, "c_cusno2", dicEXPInfo.GetValueParser<string>("v_nama", string.Empty), dicEXPInfo.GetValueParser<string>("c_cusno2", string.Empty));
                ...
                //X.AddScript(string.Format("clearRelatedComboRecursive(true, {0});", cbRSDtl.ClientID));
    ...
    private PostDataParser.StructureResponse SaveParser(bool isAdd, bool isConfirm, string plNumber, Dictionary<string, string>[] dics, Dictionary<string, string>[] dics2)
    {
        ...
        pair.DicAttributeValues.Add("BiayaLain", txBiayaLain.Text.Trim());
        ...
        pair.DicAttributeValues.Add("isCabang", "false");
        pair.DicAttributeValues.Add("Customer2", cbPrincipalHdr .SelectedItem.Value);
        ...
        for (int nLoop = 0, nLen = dics.Length; nLoop < nLen; nLoop++)
        {
            ...
            if (isNew && (!isVoid) && (!isModify))
            {
                ...
                rsno = dicData.GetValueParser<string>("c_dono");
    ...
    private void CalculateKoliBerat(string sNo, bool add, string gridValues, string cusno, string gridValues2)
    {
        ...
        try
        {
                        ...
                        if (!isNotDouble)
                        {
                            ...
                            sb.AppendFormat(@"{0}.insert(0, new Ext.data.Record({{
                              ...
                              'l_new': true,
                                                  ...
                                                  sValue, i.ToString("000"), dBerat, dKoli, dVol);
            ...
            if (!string.IsNullOrEmpty(dKoli.ToString()) && !(string.IsNullOrEmpty(dBerat.ToString())) && !(string.IsNullOrEmpty(dVol.ToString())) && !isNotDouble)
            {
                ...
                sb.AppendFormat(@"{0}.insert(0, new Ext.data.Record({{
                              ...
                              'c_nopart': '{3}',
                                      ...
                                      dKoli, dBerat, i.ToString("000"),dVol);
    ...
    protected void SaveBtn_Click(object sender, DirectEventArgs e)
    {
        ...
        if (respon.IsSet)
        {
            if (respon.Response == PostDataParser.ResponseStatus.Success)
            {
                ...
                string cust = (cbPrincipalHdr.SelectedItem != null ? cbPrincipalHdr.SelectedItem.Text : string.Empty);
                ...
                'c_resi': '{7}',
                                     ...
                                     dateJs, this.ActiveGudangDescription, cust, viaDesc, cbEksHdr.SelectedItem.Text, txNoResiHdr.Text);
            ...
            if (!isExist)
            {
                ...
                if (!isChecklist)
                {
                    ...
                    sb1.AppendFormat(@"{0}.insert(0, new Ext.data.Record({{
                                    ...
                                    'c_nopart': '{1}',
                                    'l_new': true,
                                  }})); ", gridDetail2.GetStore().ClientID, sNopart);
                    ...
                    sb.AppendFormat(@"{0}.insert(0, new Ext.data.Record({{
                                    ...
                                    'c_nopart': '{2}',
                                    'l_new': true,
                                  }})); ", gridDetail.GetStore().ClientID, s,sNopart);
    ...
    protected void RecalcBtn(object sebder, DirectEventArgs e)
    {
        ...
        {
            if (cbByHdr.SelectedItem.Value == "01")
            {
                string jsonGridValues = (e.ExtraParams["Grid"] ?? string.Empty);
