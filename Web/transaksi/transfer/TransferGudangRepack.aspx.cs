...
public partial class transaksi_transfer_TransferGudangRepack : Scms.Web.Core.PageHandler
{
  ...
  protected void Page_Load(object sender, EventArgs e)
  {
    ...
    if (!this.IsPostBack)
    {
      ...
      if (hfMode.Text.Equals(CONFIRM_MODE))
      {
        ...
        TransferGudangRepackConfirmCtrl.Initialize(pag.ActiveGudang, pag.ActiveGudangDescription, storeGridSJ.ClientID, true);
      }
      else
      {
        TransferGudangRepackCtrl.Initialize(pag.ActiveGudang, pag.ActiveGudangDescription, storeGridSJ.ClientID, false);
      }
    }
  }
  ...
  private PostDataParser.StructureResponse DeleteParser(string sjNumber, string ket)
  {
    ...
    {
      Scms.Web.Common.Logger.WriteLine("transaksi_penjualan_TransferGudangRepack DeleteParser : {0} ", ex.Message);
    }
  ...
  protected void btnAddNew_OnClick(object sender, DirectEventArgs e)
  {
    ...
    TransferGudangRepackCtrl.CommandPopulate(true, null);
  ...
  protected void gridMainCommand(object sender, DirectEventArgs e)
  {
    ...
    if (cmd.Equals("Select", StringComparison.OrdinalIgnoreCase))
    {
      if (hfMode.Text.Equals(CONFIRM_MODE))
      {
        TransferGudangRepackConfirmCtrl.CommandPopulate(false, pID);
      }
      else
      {
        TransferGudangRepackCtrl.CommandPopulate(false, pID);
      }
    }
    GC.Collect();
  }
