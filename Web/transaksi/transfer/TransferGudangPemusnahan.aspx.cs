...
public partial class transaksi_transfer_TransferGudangPemusnahan : Scms.Web.Core.PageHandler
{
  ...
  protected void Page_Load(object sender, EventArgs e)
  {
    ...
    if (!this.IsPostBack)
    {
      ...
      if (hfMode.Text.Equals(CONFIRM_MODE))
      {
        ...
        TransferGudangPemusnahanConfirmCtrl.Initialize(pag.ActiveGudang, pag.ActiveGudangDescription, storeGridSJ.ClientID, true, hfMode.Text);
      }
      else
      {
          TransferGudangPemusnahanCtrl.Initialize(pag.ActiveGudang, pag.ActiveGudangDescription, storeGridSJ.ClientID, false, hfMode.Text);
  ...
  private PostDataParser.StructureResponse DeleteParser(string sjNumber, string ket)
  {
    ...
    {
        Scms.Web.Common.Logger.WriteLine("transaksi_penjualan_TransferGudangPemusnahan DeleteParser : {0} ", ex.Message);
    }
  ...
  protected void btnAddNew_OnClick(object sender, DirectEventArgs e)
  {
    ...
    TransferGudangPemusnahanCtrl.CommandPopulate(true, null);
  ...
  protected void gridMainCommand(object sender, DirectEventArgs e)
  {
    ...
    if (cmd.Equals("Select", StringComparison.OrdinalIgnoreCase))
    {
      if (hfMode.Text.Equals(CONFIRM_MODE))
      {
        TransferGudangPemusnahanConfirmCtrl.CommandPopulate(false, pID);
      }
      else
      {
          TransferGudangPemusnahanCtrl.CommandPopulate(false, pID);
      }
    }
    GC.Collect();
  }
