...
public partial class transaksi_transfer_TransferKarantinaGudang : Scms.Web.Core.PageHandler
{
  ...
  protected void Page_Load(object sender, EventArgs e)
  {
    ...
    if (!this.IsPostBack)
    {
      ...
      if (hfMode.Text.Equals(CONFIRM_MODE))
      {
        ...
        TransferKarantinaGudangConfirmCtrl.Initialize("4", "Gudang Karantin", storeGridSJ.ClientID, true, hfMode.Text);
      }
      else
      {
          TransferKarantinaGudangCtrl.Initialize("4", "Gudang Karantin", storeGridSJ.ClientID, false, hfMode.Text);
  ...
  private PostDataParser.StructureResponse DeleteParser(string sjNumber, string ket)
  {
    ...
    {
        Scms.Web.Common.Logger.WriteLine("transaksi_penjualan_TransferKarantinaGudang DeleteParser : {0} ", ex.Message);
    }
  ...
  protected void btnAddNew_OnClick(object sender, DirectEventArgs e)
  {
    ...
    TransferKarantinaGudangCtrl.CommandPopulate(true, null);
  ...
  protected void gridMainCommand(object sender, DirectEventArgs e)
  {
    ...
    if (cmd.Equals("Select", StringComparison.OrdinalIgnoreCase))
    {
      if (hfMode.Text.Equals(CONFIRM_MODE))
      {
        TransferKarantinaGudangConfirmCtrl.CommandPopulate(false, pID);
      }
      else
      {
          TransferKarantinaGudangCtrl.CommandPopulate(false, pID);
      }
    }
    GC.Collect();
  }
