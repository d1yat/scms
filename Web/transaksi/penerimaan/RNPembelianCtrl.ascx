<%@ Control Language="C#" AutoEventWireup="true" CodeFile="RNPembelianCtrl.ascx.cs"
  Inherits="transaksi_penerimaan_RNPembelianCtrl" %>
...
<script type="text/javascript">
  ...
  var onChangePo = function(o, valu, tQty, grid, itm, qtyTemp) {
...
//        tQty.setRawValue(r.get('n_sisa'));
  ...
  var storeToDetailGrid = function(frm, grid, tipe, item, nomorPo, batch, tglbatch, quantity, qtyTemp) {
    ...
    if (!isDup) {
      ...
      store.insert(0, new Ext.data.Record({
        ...
        'c_batch': batch.getValue().toUpperCase(),
...
var validateRadio = function(rdReguler, rdNoSerahTerimaTiket) {
    if (rdReguler) {
        rdNoSerahTerimaTiket.setVisible(true);
    } else {
        rdNoSerahTerimaTiket.setVisible(false);
    };
};
...
<ext:Window ID="winDetail" runat="server" Height="480" Width="825" Hidden="true"
  ...
  <Items>
	...
	<%--Indra 20180927FM
    ST Tiket RN Pembelian--%>
      <North MinHeight="200" MaxHeight="200" Collapsible="false">
        <ext:FormPanel ID="frmHeaders" runat="server" Title="Header" Height="200" Padding="10" Layout="Column">
          <Items>
            ...
            <ext:Panel runat="server" Border="false" Header="false" ColumnWidth=".5" Layout="Form">
              <Items>
                ...
                <%--Indra 20180927FM
                ST Tiket RN Pembelian--%>
                <ext:RadioGroup ID="rdgTipeRN" runat="server" ColumnsNumber="1" FieldLabel="Tipe RN">
                  <Items>
                    <ext:Radio ID="rdReguler" runat="server" Checked="true" BoxLabel="Reguler" />
                    <ext:Radio ID="rdAdmAuto" runat="server" BoxLabel="Auto / ADM" />
                  </Items>
                  <Listeners>
                       <Change Handler="validateRadio(#{rdReguler}.getValue(), #{txNoSerahTerimaTiket});" />
                  </Listeners>
                </ext:RadioGroup>
                <ext:TextField ID="txNoSerahTerimaTiket" runat="server" FieldLabel="No. STT" MaxLength="10" Width="100">
                    <ToolTips>
                        <ext:ToolTip ID="NoSTT" runat = "server" Title="Nomor serah terima tiket transportasi"></ext:ToolTip>
                    </ToolTips>
                </ext:TextField>
              </Items>
            </ext:Panel>
                ...
                <ext:FormPanel ID="frmpnlDetailEntry" runat="server" Frame="True" Layout="Table"
                  ...
                  <Items>
                    <ext:ComboBox ID="cbTipeDtl" runat="server" FieldLabel="Tipe" DisplayField="v_ket"
                      ValueField="c_type" Width="70" AllowBlank="false" ForceSelection="false">
                      <CustomConfig>
                        <ext:ConfigItem Name="allowBlank" Value="false" />
                      </CustomConfig>
                      <Store>
                      ...
                      <Store>
                        <ext:Store runat="server">
                          ...
                          <BaseParams>
                            <ext:Parameter Name="start" Value="={0}" />
                            <ext:Parameter Name="limit" Value="={10}" />
                            <ext:Parameter Name="model" Value="10001" />
                    ...
                    <ext:ComboBox ID="cbPODtl" runat="server" FieldLabel="Nomor PO" DisplayField="c_pono"
                          ...
                          <Reader>
                            <ext:JsonReader IDProperty="c_pono" Root="d.records" SuccessProperty="d.success"
                              ...
                              <Fields>
                                ...
                                <ext:RecordField Name="v_cunam" /> <%--20170116 Indra.D--%>
                              </Fields>
                            </ext:JsonReader>
                          </Reader>
                        </ext:Store>
                      ...
                      <Template runat="server">
                        ...
                        <tr>
                        <td class="body-panel">Nomor</td>
                        <td class="body-panel">Tanggal</td>
                        <td class="body-panel">Sisa</td>
                        <%--20170116 Indra.D--%>
                        <td class="body-panel">Cabang</td>
                        <%--20170116 Indra.D--%>
                        </tr>
                        <tpl for="."><tr class="search-item">
                        <td>{c_pono}</td>
                        <td>{d_podate:this.formatDate}</td>
                        <td>{n_sisa}</td>
                        <td>{v_cunam}</td> <%--20170116 Indra.D--%>
                        </tr></tpl>
                        </table>
                        </Html>
                        <Functions>
                      ...
                      <Listeners>
                        <TriggerClick Handler="if(index == 0) { reloadData(this); }" />
                        <Change Handler="onChangePo(this, newValue, #{txQtyDtl}, #{gridDetail}, #{cbItemDtl}, #{hfQty});" />
                      </Listeners>
                    </ext:ComboBox>
                    ...
                    <ext:Button ID="btnAdd" runat="server" FieldLabel="&nbsp;" LabelSeparator=" " ToolTip="Add"
                      Icon="Add">
                      <Listeners>
                        <Click Handler="storeToDetailGrid(#{frmpnlDetailEntry}, #{gridDetail}, #{cbTipeDtl}, #{cbItemDtl}, #{cbPODtl}, #{txBatchDtl}, #{txDateDtl}, #{txQtyDtl} , #{hfQty});" />
                      </Listeners>
                    </ext:Button>
                ...
                <ext:Store runat="server" RemotePaging="false" RemoteSort="false">
                  ...
                  <BaseParams>
                    ...
                    <ext:Parameter Name="parameters" Value="[['gudang', '1', 'System.Char'],
                      ...
                      ['c_typern = @0', '01', 'System.String']]" Mode="Raw" />
                  </BaseParams>
                  <Reader>
      ...
      <DirectEvents>
        <Click OnEvent="SaveBtn_Click" Before="return verifyHeaderAndDetail(#{frmHeaders},#{gridDetail});">
          ...
          <ExtraParams>
            ...
            <%--Indra 20180927FM
            ST Tiket RN Pembelian--%>
            <ext:Parameter Name="NoSerahTerimaTiket" Value="#{txNoSerahTerimaTiket}.getValue()" Mode="Raw" />            
            <ext:Parameter Name="Reguler" Value="#{rdReguler}.getValue()" Mode="Raw" />            
