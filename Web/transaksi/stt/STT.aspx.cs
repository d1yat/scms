...
public partial class transaksi_stt_STT : Scms.Web.Core.PageHandler
{
  private const string DONASI_MODE = "01";
  private const string SAMPLE_MODE = "02";
  ...
  private PostDataParser.StructureResponse DeleteParser(string plNumber, string ket)
  {
    ...
    try
    {
      varData = parser.ParserData("STT", "Delete", dic);
    }
    catch (Exception ex)
    {
      Scms.Web.Common.Logger.WriteLine("transaksi_stt_STT DeleteParser : {0} ", ex.Message);
    }
  ...
  public void DeleteMethod(string sttNumber, string keterangan)
  {
    ...
    {
      Functional.ShowMsgWarning("Nomor STT tidak terbaca.");
    ...
    if (respon.Response == PostDataParser.ResponseStatus.Success)
    {
      ...
      Functional.ShowMsgInformation(string.Format("Nomor STT '{0}' telah terhapus.", sttNumber));
    }
    else
    {
  ...
  protected void Page_Init(object sender, EventArgs e)
  {
    if (!this.IsPostBack)
    {
      ...
      {
        hfMode.Text = SAMPLE_MODE;
      }
      else
      {
        hfMode.Text = DONASI_MODE;
      }
    }
  }
  ...
  protected void Page_Load(object sender, EventArgs e)
  {
    if (!this.IsPostBack)
    {
      STTCtrl1.Initialize(this.ActiveGudang, this.ActiveGudangDescription, storeGridSTT.ClientID, hfMode.Text);
  ...
  protected void btnAddNew_OnClick(object sender, EventArgs e)
  {
    ...
    STTCtrl1.CommandPopulate(true, null);
  }
  ...
  protected void gridMainCommand(object sender, DirectEventArgs e)
  {
    ...
    {
      STTCtrl1.CommandPopulate(false, sttID);
    }
  ...
  protected void btnPrintPL_OnClick(object sender, DirectEventArgs e)
  {
    ...
    STTCtrlPrint1.ShowPrintPage(hfMode.Text);
  }
}
