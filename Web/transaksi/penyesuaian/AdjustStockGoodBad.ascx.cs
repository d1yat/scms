...
public partial class transaksi_penyesuaian_AdjustStockGoodBad : System.Web.UI.UserControl
{
  private void ClearEntrys()
  {
    hfADJGoodBadNo.Clear();
  ...
  private PostDataParser.StructureResponse SaveParser(bool isAdd, string rnNumber, Dictionary<string, string>[] dics, string gudangID,  string keterangan)
  {
    ...
    try
    {
      varData = parser.ParserData("ADJGOODBAD", (isAdd ? "Add" : "Modify"), dic);
    }
    catch (Exception ex)
    {
      Scms.Web.Common.Logger.WriteLine("transaksi_penerimaan_ADJGOODBADCtrl SaveParser : {0} ", ex.Message);
    }
  ...
  public void CommandPopulate(bool isAdd, string pID, string gdgID)
  {
    if (isAdd)
    {
      ...
      winDetail.Title = "Adjustment Good Bad";
          ...
          if (respon.Values != null)
          {
            ...
            if (!string.IsNullOrEmpty(strStoreID))
            {
              scrpt = string.Format(@"{0}.insert(0, new Ext.data.Record({{
                ...
                'c_adjno': '{2}',
                'd_adjdate': {3},
                'ketType': 'Perpindahan status Bad <-> Good',
                'v_ket': '{4}'
