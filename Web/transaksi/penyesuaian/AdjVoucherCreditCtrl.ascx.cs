...
public partial class transaksi_penyesuaian_AdjVoucherCreditCtrl : System.Web.UI.UserControl
{
  #region Private
  ...
  private void ClearEntrys()
  {
    ...
    txKeteranganHdrCR.Clear();
    txKeteranganHdrCR.Disabled = false;

    cbTipeCR.Clear();
    cbTipeCR.Disabled = false;

    cbNoRefDtlCR.Clear();
    cbNoRefDtlCR.Disabled = false;

    txValueCR.Clear();
    txValueCR.Disabled = false;
    txKetDetCR.Clear();
    txKetDetCR.Disabled = false;
  ...
  private void PopulateDetail(string pName, string pID, string type)
  {
    ...
    try
    {
      ...
      if (dicADJ.ContainsKey("records") && (dicADJ.ContainsKey("totalRows") && (((long)dicADJ["totalRows"]) > 0)))
      {
        ...
        txKeteranganHdrCR.Text = (dicADJInfo.ContainsKey("v_ket") ? dicADJInfo["v_ket"] : string.Empty);
      }
    }
    ...
    cbTipeCR.Disabled = true;
    cbNoRefDtlCR.Disabled = true;
    txKetDetCR.Disabled = true;
    txValueCR.Disabled = true;
  ...
  private PostDataParser.StructureResponse SaveParser(bool isAdd, string adjNumber, string Keterangan, Dictionary<string, string>[] dics)
  {
    ...
    pair.DicAttributeValues.Add("Type", "17");
    ...
    {
      Scms.Web.Common.Logger.WriteLine("transaksi_penerimaan_ADJVoucherCreditCtrl SaveParser : {0} ", ex.Message);
    }
  ...
  {
    hfStoreIDCR.Text = storeIDGridMain;
    hfTypeCR.Text = "02";
  }
  ...
  public void CommandPopulate(bool isAdd, string pID, string gdgID, string Type)
  {
    ...
    if (isAdd)
    {
      ...
      winDetail.Title = "Adjustment Voucher Credit";
