...
public partial class transaksi_penyesuaian_AdjustStockStock : System.Web.UI.UserControl
{
  ...
  private void PopulateDetail(string pName, string pID, string Case)
  {
    ...
    try
    {
      ...
      if (dicADJ.ContainsKey("records") && (dicADJ.ContainsKey("totalRows") && (((long)dicADJ["totalRows"]) > 0)))
      {
        ...
        winDetail.Title = string.Format("Adjust Stock List - {0}", pID);

        //cbBatDtl.Disabled = true;
        //cbItemDtl.Disabled = true;
        //txGQtyDtl.Disabled = true;
        //txBQtyDtl.Disabled = true;
        //txKetDtl.Disabled = true;
      }
    }
    catch (Exception ex)
    {
      System.Diagnostics.Debug.WriteLine(
        string.Concat("transaction_sales_AdjustStock:PopulateDetail Header - ", ex.Message));
    }
    finally
    {
  ...
  private PostDataParser.StructureResponse SaveParser(bool isAdd, string rnNumber, Dictionary<string, string>[] dics, string gudangID, string keterangan)
  {
    ...
    pair.DicAttributeValues.Add("Type", "03");
    ...
    try
    {
      varData = parser.ParserData("ADJSTOCK", (isAdd ? "Add" : "Modify"), dic);
    }
    catch (Exception ex)
    {
      Scms.Web.Common.Logger.WriteLine("transaksi_penerimaan_ADJSTOCKCtrl SaveParser : {0} ", ex.Message);
    }
  ...
  public void CommandPopulate(bool isAdd, string pID, string gdgID)
  {
    ...
    {
      ClearEntrys();
      winDetail.Title = "Adjustment Stock";
