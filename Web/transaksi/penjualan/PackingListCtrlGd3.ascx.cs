...
public partial class transaksi_penjualan_PackingListCtrlGd3 : System.Web.UI.UserControl
{
  #region Private
...
//    txPeriode2.Text = DateTime.Now.AddDays(28).ToString("dd-MM-yyyy");
  ...
  private PostDataParser.StructureResponse SaveParser(bool isAdd, string plNumber, Dictionary<string, string>[] dics)
  {
    ...
    if (!string.IsNullOrEmpty(varData))
    {
      ...
      result = soa.PostData(varData,true);
    ...
    if (respon.IsSet)
    {
      if (respon.Response == PostDataParser.ResponseStatus.Success)
      {
        ...
        if (isAdd)
        {
          ...
          {
              respon.Message = "Data telah di proses, mohon tunggu beberapa saat.";
              respon.Response = PostDataParser.ResponseStatus.Success;
              ClearEntrys();
...
//            PackingListAutoGeneratorCtrlDetil.CommandPopulate(true, numberId, sIdPLVia, Supplier);
          }
        }
//        else if (isConfirmed)
//        {
