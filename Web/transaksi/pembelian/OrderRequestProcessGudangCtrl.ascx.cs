...
public partial class transaksi_pembelian_OrderRequestProcessGudangCtrl : System.Web.UI.UserControl
{
    #region Private
    ...
    {
        winDetail.Title = "Order Request Process Gudang";
    ...
    private void PopulateDetail(string pName, string pID)
    {
        ...
        try
        {
            ...
            if (dicResult.ContainsKey("records") && (dicResult.ContainsKey("totalRows") && (((long)dicResult["totalRows"]) > 0)))
            {
                ...
                winDetail.Title = string.Format("Order Request Process Gudang - {0}", pID);
        ...
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine(
              string.Concat("transaksi_pembelian_transaksi_pembelian_OrderRequestProcessGudangCtrl:PopulateDetail Header - ", ex.Message));
        }
        finally
        {
        ...
        {
            System.Diagnostics.Debug.WriteLine(
              string.Concat("transaksi_pembelian_transaksi_pembelian_OrderRequestProcessGudangCtrl:PopulateDetail Detail - ", ex.Message));
        }
    ...
    private PostDataParser.StructureResponse SaveParser(Dictionary<string, string>[] dics, string suplierId, string divPrinsipal, string via, string tipeProduk)
    {
        ...
        pair.DicAttributeValues.Add("Tipe", "00");
        ...
        pair.DicAttributeValues.Add("Gudang", "2");
        pair.DicAttributeValues.Add("GudangTo", "1");
        ...
        for (int nLoop = 0, nLen = dics.Length; nLoop < nLen; nLoop++)
        {
            ...
            oRder = dicData.GetValueParser<decimal>("n_qty");
        ...
        try
        {
            varData = parser.ParserData("OrderRequestProcessGudang", "Add", dic);
        }
        catch (Exception ex)
        {
            Scms.Web.Common.Logger.WriteLine("transaksi_pembelian_transaksi_pembelian_OrderRequestProcessGudangCtrl SaveParser : {0} ", ex.Message);
        }
    ...
    private List<Dictionary<string, string>> SummaryData(List<Dictionary<string, string>> listDic)
    {
        ...
        if (dicData.Count > 0)
        {
            ...
            this.Application[item] = listDic;
        }

        return listD;
    }

    private List<Dictionary<string, string>> DataToExcell(List<Dictionary<string, string>> listDic)
    {
        if (listDic == null)
        {
            return null;
        ...
        for (int nLoop = 0, nLen = listDic.Count; nLoop < nLen; nLoop++)
        {
            ...
            foreach (KeyValuePair<string, string> kvp in dicRead)
            {
                if (kvp.Key.ToLower() == "c_iteno")
                {
                    dicWrite.Add("Kode", kvp.Value);
                }
                if (kvp.Key.ToLower() == "v_itnam")
                {
                    dicWrite.Add("NamaBarang", kvp.Value);
                }
                if (kvp.Key.ToLower() == "n_qty")
                {
                    dicWrite.Add("Qty", kvp.Value);
                }
                if (kvp.Key.ToLower() == "n_spacc")
                {
                    dicWrite.Add("TotalSPdiTerima", kvp.Value);
                }
                if (kvp.Key.ToLower() == "n_avgsls")
                {
                    dicWrite.Add("AvgSales", kvp.Value);
                }
                if (kvp.Key.ToLower() == "n_soh")
                {
                    dicWrite.Add("SOH", kvp.Value);
                }
                if (kvp.Key.ToLower() == "n_sit")
                {
                    dicWrite.Add("SIT", kvp.Value);
                }
                if (kvp.Key.ToLower() == "n_salpri")
                {
                    dicWrite.Add("HNA", kvp.Value);
                }
                if (kvp.Key.ToLower() == "total")
                {
                    dicWrite.Add("Total", kvp.Value);
                }
            }
    ...
    private Dictionary<string, string>[] MergeData(string contentId, Dictionary<string, string>[] dicData)
    {
        ...
        listDic = this.Application[contentId] as List<Dictionary<string, string>>;
        if (listDic == null)
        {
            return null;
        }
    ...
    private void ClearSessionObject(string contId)
    {
        ...
        if (!string.IsNullOrEmpty(contId))
        {
            ...
            listDic = this.Application[contId] as List<Dictionary<string, string>>;
            ...
            if (listDic != null)
            {
                ...
                this.Application.Remove(contId);
            }
        }
    ...
    protected void SaveBtn_Click(object sender, DirectEventArgs e)
    {
        ...
        PostDataParser.StructureResponse respon = SaveParser(gridDataDetails, suplierId, divPrincipalValue, viaValue, tipeProdukValue);
    ...
    protected void ProcessORP_Click(object sender, DirectEventArgs e)
    {
        ...
        List<Dictionary<string, string>> lstExcellInfo = null;
        Newtonsoft.Json.Linq.JArray jarr = null;

        string[][] paramX = new string[][]{
        new string[] { "activeGudang", pag.ActiveGudang, "System.Char"},
        ...
        string result = soa.GlobalQueryService(0, -1, true, string.Empty, string.Empty, "8095", paramX);
        ...
        if (!string.IsNullOrEmpty(result))
        {
            ...
            if (dicResult.ContainsKey("records") && (dicResult.ContainsKey("totalRows") && (((long)dicResult["totalRows"]) > 0)))
            {
                ...
                lstExcellInfo = DataToExcell(lstResultInfo);
                obj = new JavaScriptSerializer().Serialize(lstExcellInfo);
            ...
            txGridTotal.Text = string.Format("{0:N}", total);
            hfTotal.Text = total.ToString();
        }
    ...
    protected void ExportEt(object sender, DirectEventArgs e)
    {
        if (obj != null)
        {
            StoreSubmitDataEventArgs eSubmit = new StoreSubmitDataEventArgs(obj.ToString(), null);
            System.Xml.XmlNode xml = eSubmit.Xml;
            string fileName = e.ExtraParams["SuplierName"] + "_" + ((Scms.Web.Core.PageHandler)this.Page).Nip + ".xls";
            this.Response.Clear();
        ...
        {
            Functional.ShowMsgInformation("Data grid kosong.");
        }
    }
}