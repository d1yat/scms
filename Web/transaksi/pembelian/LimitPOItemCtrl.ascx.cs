...
public partial class transaksi_pembelian_LimitPOItemCtrl : System.Web.UI.UserControl
{
  #region Private

  private void ClearEntrys()
  {
    winDetail.Title = "Limit Item";
  ...
  private void PopulateDetail(string pName, string pID, string tahun, string bulan)
  {
    ...
    try
    {
        ...
        if (dicLim.ContainsKey("records") && (dicLim.ContainsKey("totalRows") && (((long)dicLim["totalRows"]) > 0)))
        {
            ...
            winDetail.Title = string.Format("Limit PO Item - {0} - {1}", pID, dicLimInfo.GetValueParser<string>("v_nmdivpri", string.Empty));
    ...
    catch (Exception ex)
    {
        System.Diagnostics.Debug.WriteLine(
          string.Concat("transaksi_pembelian_Limit_PO_Item_Ctrl:PopulateDetail Header - ", ex.Message));
    }
    finally
    {
        if (jarr != null)
  ...
  private PostDataParser.StructureResponse SaveParser(bool isAdd, string idNumber, Dictionary<string, string>[] dics, string nTahun, string nBulan, decimal limitdiv, decimal limitalokasi)
  {
    ...
    pair.DicAttributeValues.Add("limit", limitdiv.ToString());
    ...
    try
    {
      varData = parser.ParserData("LimitPOItem", (isAdd ? "Add" : "Modify"), dic);
    }
    catch (Exception ex)
    {
  ...
  {
      PopulateDetail("c_kddivpri", pID, tahun, bulan);
  }
  ...
  protected void SaveBtn_Click(object sender, DirectEventArgs e)
  {
    ...
    if (respon.IsSet)
    {
      if (respon.Response == PostDataParser.ResponseStatus.Success)
      {
        ...
        //this.ClearEntrys();        
        PopulateDetail("c_kddivpri", numberId, nTahun, nBulan);
