...
public partial class transaksi_pembelian_OrderRequestGudangCtrl : System.Web.UI.UserControl
{
  #region Private
  ...
  private void ClearEntrys()
  {
    ...
    winDetail.Title = "Order Request Gudang";
  ...
  private void PopulateDetail(string pName, string pID)
  {
    ...
    string res = soa.GlobalQueryService(0, 1, false, string.Empty, string.Empty, "0015", paramX);
    ...
    try
    {
      ...
      if (dicResult.ContainsKey("records") && (dicResult.ContainsKey("totalRows") && (((long)dicResult["totalRows"]) > 0)))
      {
        ...
        winDetail.Title = string.Format("Order Request Gudang - {0}", pID);

        //cbGudangFromHdr.ToBuilder().AddItem(
        //  (dicResultInfo.ContainsKey("v_gudang_from") ? dicResultInfo["v_gudang_from"] : string.Empty),
        //  (dicResultInfo.ContainsKey("c_gdg1") ? dicResultInfo["c_gdg1"] : string.Empty)
        //);
        //if (cbGudangFromHdr.GetStore() != null)
        //{
        //  cbGudangFromHdr.GetStore().CommitChanges();
        //}
        //cbGudangFromHdr.SetValueAndFireSelect((dicResultInfo.ContainsKey("c_gdg1") ? dicResultInfo["c_gdg1"] : string.Empty));

        Functional.SetComboData(cbGudangFromHdr, "c_gdg", dicResultInfo.GetValueParser<string>("v_gudang_from", string.Empty), dicResultInfo.GetValueParser<string>("c_gdg1", string.Empty));
        cbGudangFromHdr.Disabled = true;

        //cbGudangToHdr.ToBuilder().AddItem(
        //  (dicResultInfo.ContainsKey("v_gudang_to") ? dicResultInfo["v_gudang_to"] : string.Empty),
        //  (dicResultInfo.ContainsKey("c_gdg2") ? dicResultInfo["c_gdg2"] : string.Empty)
        //);
        //if (cbGudangToHdr.GetStore() != null)
        //{
        //  cbGudangToHdr.GetStore().CommitChanges();
        //}
        //cbGudangToHdr.SetValueAndFireSelect((dicResultInfo.ContainsKey("c_gdg2") ? dicResultInfo["c_gdg2"] : string.Empty));

        Functional.SetComboData(cbGudangToHdr, "c_gdg", dicResultInfo.GetValueParser<string>("v_gudang_to", string.Empty), dicResultInfo.GetValueParser<string>("c_gdg2", string.Empty), true);
        ...
        isComplete = dicResultInfo.GetValueParser<bool>("l_status");

        txKeterangan.Text = dicResultInfo.GetValueParser<string>("v_ket", string.Empty);
        
        DateTime date = DateTime.Today;
        date = Functional.JsonDateToDate(dicResultInfo.GetValueParser<string>("d_sgsender", string.Empty));
        txDateSender.SetValue(date.ToString("dd-MM-yyyy"));
    ...
    catch (Exception ex)
    {
      System.Diagnostics.Debug.WriteLine(
        string.Concat("transaksi_pembelian_OrderRequestGudangCtrl:PopulateDetail Header - ", ex.Message));
    }
    finally
    {
    ...
    try
    {
      ...
      if (store.Proxy.Count > 0)
      {
        ...
        if ((stp != null) && stp.CallbackParam.Equals(Functional.NAME_SOA_SCMS_CALLBACK, StringComparison.OrdinalIgnoreCase))
        {
          ...
          {
            store.BaseParams["parameters"] = string.Format("[['orgno', '{1}', 'System.String']]", pName, pID);
          }
        }
      }
    ...
    {
      System.Diagnostics.Debug.WriteLine(
        string.Concat("transaksi_pembelian_OrderRequestGudangCtrl:PopulateDetail Detail - ", ex.Message));
    }
  ...
  private PostDataParser.StructureResponse SaveParser(bool isAdd, string plNumber, Dictionary<string, string>[] dics, string gdgFrom, string gdgTo, string suplierId, string keteranganHeader, DateTime dateSender)
  {
    ...
    pair.DicAttributeValues.Add("From", gdgFrom);
    pair.DicAttributeValues.Add("To", gdgTo);
    ...
    if (dateSender != DateTime.MinValue)
    {
        pair.DicAttributeValues.Add("TanggalSend", dateSender.ToString("yyyyMMddHHmmssfff"));
    }
    ...
    for (int nLoop = 0, nLen = dics.Length; nLoop < nLen; nLoop++)
    {
      ...
      isReset = dicData.GetValueParser<bool>("l_reset");

      if (isNew && (!isVoid) && (!isModify) && (!isReset))
      {
        ...
        dicAttr.Add("Reset", isReset.ToString().ToLower());
      ...
      else if ((!isNew) && isVoid && (!isModify) && (!isReset))
      {
        dicAttr = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
    ...
    try
    {
      varData = parser.ParserData("OrderRequestGudang", (isAdd ? "Add" : "Modify"), dic);
    }
    catch (Exception ex)
    {
      Scms.Web.Common.Logger.WriteLine("transaksi_pembelian_OrderRequestGudangCtrl SaveParser : {0} ", ex.Message);
    }
  ...
  protected void SaveBtn_Click(object sender, DirectEventArgs e)
  {
    ...
    string gdgFrom = (e.ExtraParams["GudangFrom"] ?? string.Empty);
    string gdgTo = (e.ExtraParams["GudangTo"] ?? string.Empty);
    string strDateSender = (e.ExtraParams["DateSender"] ?? string.Empty);

    bool isAdd = (string.IsNullOrEmpty(numberId) ? true : false);

    DateTime senderDate = DateTime.MinValue;

    Functional.DateParser(strDateSender, "yyyy-MM-ddTHH:mm:ss", out senderDate);
    ...
    else if (!pag.IsAllowEdit)
    {
      Functional.ShowMsgError("Maaf, anda tidak mempunyai hak akses untuk menyimpan data.");
      return;
    }
    else if (!gdgFrom.Equals(pag.ActiveGudang.ToString(), StringComparison.OrdinalIgnoreCase))
    {
      Functional.ShowMsgError("Maaf, anda tidak mempunyai hak akses untuk menyimpan data jika beda gudang.");
      return;
    }

    Dictionary<string, string>[] gridDataPL = JSON.Deserialize<Dictionary<string, string>[]>(jsonGridValues);

    PostDataParser.StructureResponse respon = SaveParser(isAdd, numberId, gridDataPL, gdgFrom, gdgTo, suplierId, ketHeader, senderDate);
            ...
            if (!string.IsNullOrEmpty(storeId))
            {
              scrpt = string.Format(@"{0}.insert(0, new Ext.data.Record({{
                ...
                'l_status': false
                    ...
                    respon.Values.GetValueParser<string>("SPG", string.Empty),
