...
public partial class transaksi_pembelian_LimitPODivPriCtrl : System.Web.UI.UserControl
{
  #region Private

  private void ClearEntrys()
  {
    winDetail.Title = "Limit Divisi Principal";
  ...
  private void PopulateDetail(string pName, string pID, string tahun, string bulan)
  {
    ...
    string res = soa.GlobalQueryService(0, 1, false, string.Empty, string.Empty, "05008", paramX);
    ...
    try
    {
        ...
        if (dicLim.ContainsKey("records") && (dicLim.ContainsKey("totalRows") && (((long)dicLim["totalRows"]) > 0)))
        {
            ...
            winDetail.Title = string.Format("Limit PO Divisi Principal - {0} - {1}", pID, dicLimInfo.GetValueParser<string>("v_nama", string.Empty));

            //lbLimitPrinc.Text = dicLimInfo.GetValueParser<string>("limit", string.Empty);
            //lbLimitSisaPrinc.Text = dicLimInfo.GetValueParser<string>("avaiblelimit", string.Empty);
            lbLimitPri.Text = dicLimInfo.GetValueParser<decimal>("limit", 0).ToString("N2", culture);
    ...
    {
        System.Diagnostics.Debug.WriteLine(
          string.Concat("transaksi_pembelian_LimitPODivPriCtrl:PopulateDetail Detail - ", ex.Message));
    }
  ...
  private PostDataParser.StructureResponse SaveParser(bool isAdd, string idNumber, Dictionary<string, string>[] dics, string nTahun, string nBulan, decimal limit, decimal limitalokasi)
  {
      ...
      pair.DicAttributeValues.Add("nosup", idNumber);
      pair.DicAttributeValues.Add("nTahun", nTahun);
      pair.DicAttributeValues.Add("nBulan", nBulan);
      pair.DicAttributeValues.Add("limit", limit.ToString());
      pair.DicAttributeValues.Add("limitalokasi", limitalokasi.ToString());
      ...
      for (int nLoop = 0, nLen = dics.Length; nLoop < nLen; nLoop++)
      {
          ...
          isTotal = dicData.GetValueParser<bool>("l_total");
          ...
          if (isModify && !isTotal)
          {
              ...
              kddivpri = dicData.GetValueParser<string>("c_kddivpri");

              if (!string.IsNullOrEmpty(kddivpri))
              {
                  dicAttr.Add("kddivpridtl", kddivpri);
      ...
      try
      {
          varData = parser.ParserData("LimitPODivPri", (isAdd ? "Add" : "Modify"), dic);
      }
      catch (Exception ex)
      {
          Scms.Web.Common.Logger.WriteLine("transaksi_pembelian_LimitPODivPriCtrl SaveParser : {0} ", ex.Message);
      }
  ...
  protected void SaveBtn_Click(object sender, DirectEventArgs e)
  {
      ...
      string limit = (e.ExtraParams["limit"] ?? string.Empty);
      ...
      limbudget = decimal.Parse(limit.Replace(".", "").Replace(",", "."));
