...
public partial class transaksi_pembelian_SuratPesananCtrlManual : System.Web.UI.UserControl
{
  #region Private

  private void ClearEntrys()
  {
    winDetail.Title = "Surat Pesanan Manual";

    hfSpNo.Clear();

    if (hfGudang.Text.Length < 2)
    {
        cbCustomerHdr.Clear();
  ...
  private void PopulateDetail(string pName, string pID)
  {
    ...
    winDetail.Title = string.Format("Surat Pesanan Manual- {0}", pID);
    btnSave.Hidden = true;
    ...
    //    string.Concat("transaksi_pembelian_SuratPesananCtrlManual:PopulateDetail Header - ", ex.Message));
    //}
    //finally
    //{
  ...
  private PostDataParser.StructureResponse SaveParser(bool isAdd, string plNumber, Dictionary<string, string>[] dics)
  {
    ...
    try
    {
        varData = parser.ParserData("SuratPesananManual", (isAdd ? "Add" : "Modify"), dic);
    }
    catch (Exception ex)
    {
      Scms.Web.Common.Logger.WriteLine("transaksi_pembelian_SuratPesananManualCtrl SaveParser : {0} ", ex.Message);
    }
  ...
  public void CommandPopulate(bool isAdd, string pID, string gudang, string gudangDesc)
  {
    if (isAdd)
    {
      SetEditor(false);
      ...
      if (hfGudang.Text.Length > 1)
      {
          Functional.SetComboData(cbCustomerHdr, "c_cusno", gudangDesc, gudang);
          cbCustomerHdr.Disabled = true;
      }
    ...
    else
    {
      if (rptResult.IsSuccess)
      {
        string tmpUri = Functional.UriDownloadGenerator(pag,
          rptResult.OutputFile, "Surat Pesanan Manual", rptResult.Extension);
