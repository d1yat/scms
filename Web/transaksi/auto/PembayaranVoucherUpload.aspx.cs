...
public partial class transaksi_auto_PembayaranVoucherUpload : Scms.Web.Core.PageHandler
{
  ...
  private string UploadData(bool isValidateOnly, string fileName, byte[] byts)
  {
    ...
    if ((!string.IsNullOrEmpty(fileName)) && (byts != null) && (byts.Length > 0))
    {
      ...
      result = soa.GlobalUploadData("up222", fileName, byts, paramX);
    }
    ...
    try
    {
      ...
      if (dicResult.ContainsKey("records") && (dicResult.ContainsKey("totalRows") && (dicResult.GetValueParser<long>("totalRows") > 0)))
      {
        ...
        for (nLoop = 0, nLen = listDicResultInfo.Count; nLoop < nLen; nLoop++)
        {
          ...
          lst.Add(dicResultInfo.GetValueParser<string>("c_fbno", string.Empty));
          lst.Add(dicResultInfo.GetValueParser<decimal>("n_value", 0));
          lst.Add(dicResultInfo.GetValueParser<string>("c_vdno", string.Empty));
          lst.Add(dicResultInfo.GetValueParser<string>("c_noteno", string.Empty));
          lst.Add(date.ToString("dd-MMM-yyyy"));
          lst.Add(dicResultInfo.GetValueParser<string>("c_bank", string.Empty));
