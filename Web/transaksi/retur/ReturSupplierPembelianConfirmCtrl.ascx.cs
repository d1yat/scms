...
public partial class transaksi_retur_ReturSupplierPembelianConfirmCtrl : System.Web.UI.UserControl
{
  #region Private
  ...
  private void ClearEntrys()
  {
    ...
    winDetail.Title = "Retur Supplier Pembelian Konfirmasi";

    lbGudangHdr.Text = "";
    lbPrincipalHdr.Text = "";
    lbCprNo.Text = "";
    lbKeteranganHdr.Text = "";
  ...
  private void PopulateDetail(string pName, string pID)
  {
    ...
    try
    {
      ...
      if (dicRS.ContainsKey("records") && (dicRS.ContainsKey("totalRows") && (((long)dicRS["totalRows"]) > 0)))
      {
        ...
        winDetail.Title = string.Format("Retur Supplier Pembelian Konfirmasi - {0}", pID);
        
        ////cbGudangHdr.ToBuilder().AddItem(
        ////  (dicRSInfo.ContainsKey("v_gdgdesc") ? dicRSInfo["v_gdgdesc"] : string.Empty),
        ////  (dicRSInfo.ContainsKey("c_gdg") ? dicRSInfo["c_gdg"] : string.Empty)
        ////);
        ////if (cbGudangHdr.GetStore() != null)
        ////{
        ////  cbGudangHdr.GetStore().CommitChanges();
        ////}
        ////cbGudangHdr.SetValueAndFireSelect((dicRSInfo.ContainsKey("c_gdg") ? dicRSInfo["c_gdg"] : string.Empty));
        //Functional.SetComboData(cbGudangHdr, "c_gdg", dicRSInfo.GetValueParser<string>("v_gdgdesc", string.Empty), dicRSInfo.GetValueParser<string>("c_gdg", string.Empty));
        //cbGudangHdr.Disabled = true;
        lbGudangHdr.Text = dicRSInfo.GetValueParser<string>("v_gdgdesc", string.Empty);
        
        ////cbPrincipalHdr.ToBuilder().AddItem(
        ////  (dicRSInfo.ContainsKey("v_nama") ? dicRSInfo["v_nama"] : string.Empty),
        ////  (dicRSInfo.ContainsKey("c_nosup") ? dicRSInfo["c_nosup"] : string.Empty)
        ////);
        ////if (cbPrincipalHdr.GetStore() != null)
        ////{
        ////  cbPrincipalHdr.GetStore().CommitChanges();
        ////}
        ////cbPrincipalHdr.SetValueAndFireSelect((dicRSInfo.ContainsKey("c_nosup") ? dicRSInfo["c_nosup"] : string.Empty));        
        ...
        lbPrincipalHdr.Text = dicRSInfo.GetValueParser<string>("v_nama", string.Empty);
        
        lbCprNo.Text = dicRSInfo.GetValueParser<string>("c_cprno", string.Empty);

        //txKeterangan.Text = ((dicRSInfo.ContainsKey("v_ket") ? dicRSInfo["v_ket"] : string.Empty));
        lbKeteranganHdr.Text = dicRSInfo.GetValueParser<string>("v_ket", string.Empty);
  ...
  private PostDataParser.StructureResponse SaveParser(bool isAdd, string plNumber, Dictionary<string, string>[] dics, string Gudangid, string Prinsipal, string keterangan)
  {
    ...
    pair.DicAttributeValues.Add("TipeRS", "02");
    ...
    try
    {
      varData = parser.ParserData("RSREPACK", (isAdd ? "Add" : "Modify"), dic);
    }
    catch (Exception ex)
    {
      Scms.Web.Common.Logger.WriteLine("transaksi_retur_ReturSupplierPembelianConfirmCtrl SaveParser : {0} ", ex.Message);
    }
  ...
  protected void Report_OnGenerate(object sender, DirectEventArgs e)
  {
    ...
    {
      DataType = typeof(char).FullName,
      ParameterName = "({LG_RSH.c_type} = '03')",
    ...
    else
    {
      if (rptResult.IsSuccess)
      {
        ...
        string tmpUri = Functional.UriDownloadGenerator(pag,
          rptResult.OutputFile, "Retur Suplier Pembelian Confirm ", rptResult.Extension);
