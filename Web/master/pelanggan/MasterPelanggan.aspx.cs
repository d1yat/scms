...
public partial class master_pelanggan_MasterPelanggan : Scms.Web.Core.PageHandler
{
  private PostDataParser.StructureResponse DeleteParser(string Number, string ket)
  {
    ...
    try
    {
      varData = parser.ParserData("MasterPelanngan", "Delete", dic);
    }
    catch (Exception ex)
    {
      Scms.Web.Common.Logger.WriteLine("Master_Customer DeleteParser : {0} ", ex.Message);
    }
  ...
  public void DeleteMethod(string plNumber, string keterangan)
  {
    ...
    {
      Functional.ShowMsgWarning("Nomor Customer tidak terbaca.");
  ...
  protected void Page_Load(object sender, EventArgs e)
  {
    if (!this.IsPostBack)
    {
      MasterPelangganCtrl.Initialize(storeGridItem.ClientID);
    }
  }
  ...
  protected void btnAddNew_OnClick(object sender, DirectEventArgs e)
  {
    ...
    MasterPelangganCtrl.CommandPopulate(true, null);
  }
  ...
  protected void gridMainCommand(object sender, DirectEventArgs e)
  {
    ...
    {
      MasterPelangganCtrl.CommandPopulate(false, pID);
    }
