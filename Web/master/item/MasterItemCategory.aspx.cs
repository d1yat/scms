...
public partial class master_item_MasterItemCategory : Scms.Web.Core.PageHandler
{
  #region Private
  ...
  private PostDataParser.StructureResponse SaveParser(string tipeId, Dictionary<string, string>[] dics)
  {
    ...
    try
    {
      varData = parser.ParserData("MasterItemCategory", "Modify", dic);
    }
    catch (Exception ex)
    {
      Scms.Web.Common.Logger.WriteLine("master_item_MasterItemCategory SaveParser : {0} ", ex.Message);
    }
  ...
  private PostDataParser.StructureResponse SaveMethodParser(string method, string typeNumber, string ket)
  {
    ...
    pair.DicAttributeValues.Add("Deskripsi", "Master Item Category");
    ...
    {
      Scms.Web.Common.Logger.WriteLine("master_item_MasterItemCategory DeleteParser : {0} ", ex.Message);
    }
  ...
  protected void btnAddNew_OnClick(object sender, DirectEventArgs e)
  {
    ...
    MasterItemCategoryCtrl.CommandPopulate(null, null, true);
  }
  ...
  {0}.insert(0, new Ext.data.Record({{
    ...
    'v_ket': '{3}'
      ...
      if (isDelete)
      {
        Functional.ShowMsgInformation(string.Format("Master Item Kategori '{0}' telah terhapus.", idNumber));
      }
      else
      {
        Functional.ShowMsgInformation("Data berhasil tersimpan.");
