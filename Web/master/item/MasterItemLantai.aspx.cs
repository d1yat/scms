...
public partial class master_item_MasterItemLantai : Scms.Web.Core.PageHandler
{
  #region Private
  ...
  private PostDataParser.StructureResponse SaveParser(string tipeId, Dictionary<string, string>[] dics)
  {
    ...
    try
    {
      varData = parser.ParserData("MasterItemLantai", "Modify", dic);
    }
    catch (Exception ex)
    {
      Scms.Web.Common.Logger.WriteLine("master_item_MasterItemLantai SaveParser : {0} ", ex.Message);
    }
  ...
  private PostDataParser.StructureResponse SaveMethodParser(string method, string typeNumber, string ket)
  {
    ...
    pair.DicAttributeValues.Add("TransaksiID", "003");
    pair.DicAttributeValues.Add("Deskripsi", "Master Item Lantai");
    ...
    {
      Scms.Web.Common.Logger.WriteLine("master_item_MasterItemLantai DeleteParser : {0} ", ex.Message);
    }
  ...
  protected void btnAddNew_OnClick(object sender, DirectEventArgs e)
  {
    ...
    MasterItemLantaiCtrl.CommandPopulate(null, null, true);
  }
  ...
  {0}.insert(0, new Ext.data.Record({{
    ...
    'v_ket': '{3}'
      ...
      if (isDelete)
      {
        Functional.ShowMsgInformation(string.Format("Master Item Lantai '{0}' telah terhapus.", idNumber));
      }
      else
      {
        Functional.ShowMsgInformation("Data berhasil tersimpan.");
  ...
  private PostDataParser.StructureResponse DeleteParser(string Tipe, string Item, string ket)
  {
    PostDataParser.StructureResponse responseResult = default(PostDataParser.StructureResponse);
  ...
  public void DeleteMethod(string Tipe, string Item, string keterangan)
  {
    ...
    {
      Functional.ShowMsgWarning("Nomor Tipe tidak terbaca.");
      //Ext.Net.ResourceManager.AjaxSuccess = false;
    ...
    PostDataParser.StructureResponse respon = DeleteParser(Tipe, Item, keterangan);

    if (respon.Response == PostDataParser.ResponseStatus.Success)
    {
      X.AddScript(
        string.Format("var r = {0}.getById('{1}');if(!Ext.isEmpty(r)) {{ {0}.remove(r);{0}.commitChanges(); }}",
        storeGridMasterTrx.ClientID, Item));

      Functional.ShowMsgInformation(string.Format("Nomor Item '{0}' telah terhapus.", Item));
    }
    else
    {
      Functional.ShowMsgWarning(respon.Message);
