<%@ Control Language="C#" AutoEventWireup="true" CodeFile="PackingListConf.ascx.cs" 
Inherits="reporting_monitoring_PackingListConf" %>

<ext:Panel runat="server">
  <Items>
    <ext:Hidden ID="hidWndDown" runat="server" />
    
    <ext:FormPanel ID="frmReportPLConf" runat="server" Padding="5" Frame="True" 
      Layout="Form">
      <Items>
        ...
        <ext:RadioGroup ID="rdgTipeConf" runat="server" ColumnsNumber="1" FieldLabel="Tipe PL">
          <Items>
            <ext:Radio ID="rdAll" runat="server" BoxLabel="All" Checked ="true" Tag="all" />
            <ext:Radio ID="rdConf" runat="server" BoxLabel="Confirm" Tag="cnf" />
            <ext:Radio ID="rdBelum" runat="server" BoxLabel="Belum Confirm" Tag="blm" />
          </Items>
        </ext:RadioGroup>
  ...
  <Buttons>
    <ext:Button runat="server" Icon="ReportGo" Text="Generate">
      <DirectEvents>
        <Click OnEvent="Report_OnGenerate" Before="return isValidForm(#{frmReportPLConf});">
    ...
    <ext:Button runat="server" Icon="Reload" Text="Bersihkan">
      <Listeners>
        <Click Handler="initializePanel(#{frmReportPLConf}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
      </Listeners>
    </ext:Button>
  </Buttons>
  <Listeners>
    <AfterRender Handler="initializePanel(#{frmReportPLConf}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
  </Listeners>
</ext:Panel>
