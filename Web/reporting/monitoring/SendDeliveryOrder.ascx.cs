...
public partial class reporting_monitoring_SendDeliveryOrder : System.Web.UI.UserControl
{
  ...
  protected void Report_OnGenerate(object sender, DirectEventArgs e)
  {
    ...
    rptParse.ReportingID = "21052-1";

    isAsync = chkAsync.Checked;

    tmp = txNoDO1.Text.Trim();
    tmp1 = txNoDO2.Text.Trim();
    ...
    if (Scms.Web.Common.Functional.DateParser(txPeriode1.RawText.Trim(), "d-M-yyyy", out date1))
    {
      ...
      if ((!date1.Equals(DateTime.MinValue)) && (!date1.Equals(DateTime.MinValue)) && (!date1.Equals(date2)))
      {
        ...
        {
          DataType = typeof(string).FullName,
          ParameterName = string.Format("({{LG_DOH.d_dodate}} IN {0} to {1})", Functional.CrystalReportDateString(date1), Functional.CrystalReportDateString(date2)),
    ...
    if (txNoDO1.Text.Length > 0)
    {
      if (txNoDO2.Text.Length > 0)
      {
        lstRptParam.Add(new ReportParameter()
        {
          DataType = typeof(string).FullName,
          ParameterName = string.Format("({{LG_DOH.c_dono}} IN [{0},{1}])", txNoDO1.Text, txNoDO2.Text),
          IsReportDirectValue = true
        });
      }
      else
      {
        ...
        {
          DataType = typeof(string).FullName,
          ParameterName = string.Format("({{LG_DOH.c_dono}} >= {0})", txNoDO1.Text),
          IsReportDirectValue = true
        });
      }
    }
    else if (txNoDO2.Text.Length > 0)
    {
      lstRptParam.Add(new ReportParameter()
      {
        DataType = typeof(string).FullName,
        ParameterName = string.Format("({{LG_DOH.c_dono}} <= {0})", txNoDO2.Text),
        IsReportDirectValue = true
      });
    }
    ...
    //  {
    //    DataType = typeof(string).FullName,
    //    ParameterName = string.Format("({{LG_DOD1.c_iteno}} IN [{0}])", string.Join(",", lstData.ToArray())),
    //    IsReportDirectValue = true
    //  });
    ...
    else
    {
      if (rptResult.IsSuccess)
      {
        ...
        {
          string rptName = string.Concat("Monitoring_Send_DO", pag.Nip, ".", rptResult.Extension);
