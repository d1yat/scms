<%@ Control Language="C#" AutoEventWireup="true" CodeFile="SendDeliveryOrder.ascx.cs" 
Inherits="reporting_monitoring_SendDeliveryOrder" %>

<ext:Panel ID="Panel1" runat="server">
  <Items>
    <ext:Hidden ID="hidWndDown" runat="server" />
    
    <ext:FormPanel ID="frmSendDeliveryOrder" runat="server" Padding="5" Frame="True" 
      Layout="Form">
      <Items>
        ...
        <ext:CompositeField runat="server" FieldLabel="Jarak DO">
          <Items>
            <ext:TextField ID="txNoDO1" runat="server" AllowBlank="true">
            </ext:TextField>
            <ext:Label ID="Label2" runat="server" Text="-" />
            <ext:TextField ID="txNoDO2" runat="server" AllowBlank="true">
            </ext:TextField>
          </Items>
        </ext:CompositeField>
        ...
        <ext:RadioGroup ID="rdgTipeSend" runat="server" ColumnsNumber="1" FieldLabel="Tipe Send">
          <Items>
            <ext:Radio ID="rdAll" runat="server" BoxLabel="All" Checked ="true" Tag="all" />
            <ext:Radio ID="rdSend" runat="server" BoxLabel="Terkirim" Tag="snd" />
            <ext:Radio ID="rdBlmSend" runat="server" BoxLabel="Belum Terkirim" Tag="blm" />
          </Items>
        </ext:RadioGroup>
  ...
  <Buttons>
    <ext:Button runat="server" Icon="ReportGo" Text="Generate">
      <DirectEvents>
        <Click OnEvent="Report_OnGenerate" Before="return isValidForm(#{frmSendDeliveryOrder});">
    ...
    <ext:Button runat="server" Icon="Reload" Text="Bersihkan">
      <Listeners>
        <Click Handler="initializePanel(#{frmSendDeliveryOrder}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
      </Listeners>
    </ext:Button>
  </Buttons>
  <Listeners>
    <AfterRender Handler="initializePanel(#{frmSendDeliveryOrder}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
  </Listeners>
</ext:Panel>
