<%@ Control Language="C#" AutoEventWireup="true" CodeFile="PackingList.ascx.cs" 
Inherits="reporting_monitoring_PackingList" %>

<ext:Panel runat="server">
  <Items>
    <ext:Hidden ID="hidWndDown" runat="server" />
    
    <ext:FormPanel ID="frmReportPL" runat="server" Padding="5" Frame="True" 
      Layout="Form">
      <Items>
        ...
        <ext:RadioGroup ID="rdgTipePL" runat="server" ColumnsNumber="1" FieldLabel="Tipe PL">
          <Items>
            <ext:Radio ID="rdAll" runat="server" BoxLabel="Semua" Checked ="true" Tag="all" />
            <ext:Radio ID="rdPrint" runat="server" BoxLabel="Print" Tag="prn" />
            <ext:Radio ID="rdBelum" runat="server" BoxLabel="Belum Print" Tag="blm" />
          </Items>
        </ext:RadioGroup>
  ...
  <Buttons>
    <ext:Button runat="server" Icon="ReportGo" Text="Generate">
      <DirectEvents>
        <Click OnEvent="Report_OnGenerate" Before="return isValidForm(#{frmReportPL});">
    ...
    <ext:Button runat="server" Icon="Reload" Text="Bersihkan">
      <Listeners>
        <Click Handler="initializePanel(#{frmReportPL}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
      </Listeners>
    </ext:Button>
  </Buttons>
  <Listeners>
    <AfterRender Handler="initializePanel(#{frmReportPL}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
  </Listeners>
</ext:Panel>