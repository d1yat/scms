...
public partial class reporting_pending_PurchaseOrderFinance : System.Web.UI.UserControl
{
  ...
  protected void Report_OnGenerate(object sender, DirectEventArgs e)
  {
    ...
    {
      DataType = typeof(string).FullName,
      ParameterName = "LG_PENDPOPRIN.c_user",
      ParameterValue = pag.Nip
    });
    ...
    if (!string.IsNullOrEmpty(cbDivAms.SelectedItem.Value))
    {
      ...
      {
        DataType = typeof(string).FullName,
        ParameterName = "LG_PENDPOPRIN.C_KDDIVAMS",
        ParameterValue = (cbDivAms.SelectedItem.Value == null ? string.Empty : cbDivAms.SelectedItem.Value)
      });
    }
    ...
    if (!string.IsNullOrEmpty(cbSuplier.SelectedItem.Value))
    {
      ...
      {
        DataType = typeof(string).FullName,
        ParameterName = "LG_PENDPOPRIN.c_nosup",
    ...
    if (!string.IsNullOrEmpty(cbDivPrinsipal.SelectedItem.Value))
    {
      ...
      {
        DataType = typeof(string).FullName,
        ParameterName = "LG_PENDPOPRIN.c_kddivpri",
        ParameterValue = (cbDivPrinsipal.SelectedItem.Value == null ? string.Empty : cbDivPrinsipal.SelectedItem.Value)
      });
    }
    ...
    if (!string.IsNullOrEmpty(cbItems.SelectedItem.Value))
    {
      ...
      {
        DataType = typeof(string).FullName,
        ParameterName = "LG_PENDPOPRIN.c_iteno",
    ...
    //  {
    //    DataType = typeof(string).FullName,
    //    ParameterName = string.Format("({{LG_PENDPOPRIN.C_KDDIVAMS}} IN [{0}])", string.Join(",", lstData.ToArray())),
    ...
    //  {
    //    DataType = typeof(string).FullName,
    //    ParameterName = string.Format("({{LG_PENDPOPRIN.c_nosup}} IN [{0}])", string.Join(",", lstData.ToArray())),
    //    IsReportDirectValue = true
    //  });
    ...
    //  {
    //    DataType = typeof(string).FullName,
    //    ParameterName = string.Format("({{LG_PENDPOPRIN.c_kddivpri}} IN [{0}])", string.Join(",", lstData.ToArray())),
    //    IsReportDirectValue = true
    //  });
    ...
    //  {
    //    DataType = typeof(string).FullName,
    //    ParameterName = string.Format("({{LG_PENDPOPRIN.c_iteno}} IN [{0}])", string.Join(",", lstData.ToArray())),
    //    IsReportDirectValue = true
    //  });
    ...
    rptParse.ReportCustomizeText = new ReportCustomizeText[] {
      new ReportCustomizeText()
      {
        ...
        Value = string.Format("Periode PO : {0} s/d {1}", date1.ToString("dd-MMM-yyyy"), date2.ToString("dd-MMM-yyyy"))
      }};
