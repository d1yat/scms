...
public partial class reporting_pending_ReturCustomer : System.Web.UI.UserControl
{
  ...
  protected void Report_OnGenerate(object sender, DirectEventArgs e)
  {
    ...
    if (Scms.Web.Common.Functional.DateParser(txPeriode1.RawText.Trim(), "d-M-yyyy", out date1))
    {
      ...
      if ((!date1.Equals(DateTime.MinValue)) && (!date1.Equals(DateTime.MinValue)) && (!date1.Equals(date2)))
      {
        lstRptParam.Add(new ReportParameter()
        {
          ...
          ParameterName = string.Format("({{LG_RCH.d_rcdate}} In {0} To {1})",
    ...
    if (rdLunas.Checked)
    {
        ...
        {
            DataType = typeof(string).FullName,
            ParameterName = "IsNull({LG_FJRD3.c_fjno}) = true",
            IsReportDirectValue = true
        });
    }
    else if (rdPending.Checked)
    {
        ...
        {
            DataType = typeof(string).FullName,
            ParameterName = "IsNull({LG_FJRD3.c_fjno}) = false",
            IsReportDirectValue = true
        });
    }
    ...
    if (tmp.Length > 0)
    {
        if (tmp1.Length > 0)
        {
            ...
            {
                DataType = typeof(string).FullName,
                ParameterName = string.Format("({{LG_RCH.c_rcno}} IN ['{0}','{1}'])", tmp, tmp1),
                IsReportDirectValue = true
            });
        }
        else
        {
            ...
            {
                DataType = typeof(string).FullName,
                ParameterName = string.Format("({{LG_RCH.c_rcno}} >= '{0}')", tmp),
                IsReportDirectValue = true
            });
        }
    }
    else if (tmp1.Length > 0)
    {
        ...
        {
            DataType = typeof(string).FullName,
            ParameterName = string.Format("({{LG_RCH.c_rcno}} <= '{0}')", tmp1),
            IsReportDirectValue = true
        });
    }
    ...
    else
    {
      if (rptResult.IsSuccess)
      {
        ...
        {
          string rptName = string.Concat("Pending_RC_", pag.Nip, ".", rptResult.Extension);
