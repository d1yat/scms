<%@ Control Language="C#" AutoEventWireup="true" CodeFile="PendingPOPeriodik.ascx.cs" Inherits="reporting_pending_POPeriodik" %>
      ...
      <Items>
        <ext:CompositeField ID="CompositeField1" runat="server" FieldLabel="Periode">
          <Items>
            <ext:DateField ID="txPeriode1" runat="server" Vtype="daterange" Format="dd-MM-yyyy"
              AllowBlank="false" Visible="false">
              <CustomConfig>
            ...
            <ext:Store ID="Store4" runat="server">
              ...
              <BaseParams>
                ...
                <ext:Parameter Name="parameters" Value="[['l_hide != @0', true, 'System.Boolean'],
                      ['@contains.c_kddivams.Contains(@0) || @contains.v_nmdivams.Contains(@0) || @contains.c_kddivams.Contains(@0)', paramTextGetter(#{cbDivAms}), '']]" Mode="Raw" />
        ...
        <ext:ComboBox ID="cbItems" runat="server" FieldLabel="Produk" ValueField="c_iteno"
          ...
          <Listeners>
            ...
            <BeforeQuery Handler="this.triggers[0][ this.getRawValue().toString().length == 0 ? 'hide' : 'show']();" />
            <TriggerClick Handler="if(index == 0) { this.clearValue(); this.triggers[0].hide(); } else if(index == 1) { reloadData(this); }" />
            <Change Handler="clearRelatedComboRecursive(true, #{cbBatch1}, #{cbBatch2});" />
          </Listeners>
        </ext:ComboBox>
