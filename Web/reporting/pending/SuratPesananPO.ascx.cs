...
public partial class reporting_pending_SuratPesananPO : System.Web.UI.UserControl
{
  ...
  protected void Report_OnGenerate(object sender, DirectEventArgs e)
  {
    ...
    {
      DataType = typeof(string).FullName,
      ParameterName = "Temp_RptSPPO.c_user",
      ParameterValue = pag.Nip
    });
    ...
    if (!string.IsNullOrEmpty(cbCustomer.SelectedItem.Value))
    {
      ...
      {
        DataType = typeof(string).FullName,
        ParameterName = "Temp_RptSPPO.c_cusno",
    ...
    if (!string.IsNullOrEmpty(cbSuplier.SelectedItem.Value))
    {
      ...
      {
        DataType = typeof(string).FullName,
        ParameterName = "Temp_RptSPPO.c_nosup",
    ...
    if (!string.IsNullOrEmpty(cbItems.SelectedItem.Value))
    {
      ...
      {
        DataType = typeof(string).FullName,
        ParameterName = "Temp_RptSPPO.c_iteno",
    ...
    //  {
    //    DataType = typeof(string).FullName,
    //    ParameterName = string.Format("({{Temp_RptSPPO.c_cusno}} IN [{0}])", string.Join(",", lstData.ToArray())),
    ...
    //  {
    //    DataType = typeof(string).FullName,
    //    ParameterName = string.Format("({{Temp_RptSPPO.c_nosup}} IN [{0}])", string.Join(",", lstData.ToArray())),
    //    IsReportDirectValue = true
    //  });
    ...
    //  {
    //    DataType = typeof(string).FullName,
    //    ParameterName = string.Format("({{Temp_RptSPPO.c_iteno}} IN [{0}])", string.Join(",", lstData.ToArray())),
    //    IsReportDirectValue = true
    //  });
    ...
    else
    {
      if (rptResult.IsSuccess)
      {
        ...
        {
          string rptName = string.Concat("Pending_SP_vs_PO_", pag.Nip, ".", rptResult.Extension);
