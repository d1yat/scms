...
public partial class reporting_konsolidasi_WPSuplierCabang : System.Web.UI.UserControl
{
  ...
  protected void Report_OnGenerate(object sender, DirectEventArgs e)
  {
    ...
    if (Scms.Web.Common.Functional.DateParser(txPeriode1.RawText.Trim(), "d-M-yyyy", out date1))
    {
      ...
      if (date1.Equals(date2))
      {
        lstRptParam.Add(new ReportParameter()
        {
          ...
          ParameterName = "LG_TEMPWPSupplierCabang.d_podate",
          ParameterValue = date1.ToString()
        });
      }
      else
      {
        ...
        {
          DataType = typeof(DateTime).FullName,
          ParameterName = string.Format("({{LG_TEMPWPSupplierCabang.d_podate}} In {0} To {1})", 
    ...
    if (cbCustomer.SelectedItems.Count > 1)
    {
      ...
      lstRptParam.Add(new ReportParameter()
      {
        ...
        ParameterName = string.Format("({{LG_TEMPWPSupplierCabang.c_cusno}} IN [{0}])", string.Join(",", lstData.ToArray())),
        IsReportDirectValue = true
      });
    ...
    if (cbSuplier.SelectedItems.Count > 1)
    {
      ...
      {
        DataType = typeof(string).FullName,
        ParameterName = string.Format("({{LG_TEMPWPSupplierCabang.c_nosup}} IN [{0}])", string.Join(",", lstData.ToArray())),
        IsReportDirectValue = true
      });
