<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ExpireBatch.ascx.cs"
  Inherits="reporting_Inventory_ExpireBatch" %>
  
<ext:Panel ID="Panel1" runat="server">
  <Items>
    <ext:Hidden ID="hidWndDown" runat="server" />
    <ext:FormPanel ID="frmExpireBatch" runat="server" Padding="5" Frame="True" Layout="Form">
      <Items>
        <ext:CompositeField ID="CompositeField1" runat="server" FieldLabel="Periode" hidden="true">
          <Items>
        ...
        <ext:RadioGroup ID="rdgTipeReport" runat="server" ColumnsNumber="1" FieldLabel="Tipe Laporan">
          <Items>
            <ext:Radio ID="rdgAll" runat="server" BoxLabel="Semua" Checked="true" InputValue="01" />
            <ext:Radio ID="rdgSdhExpired" runat="server" BoxLabel="Sudah Expired" InputValue="02" />
            <ext:Radio ID="rdgBlmExpired" runat="server" BoxLabel="Belum Expired" InputValue="03" />
          </Items>
        </ext:RadioGroup>
  ...
  <Buttons>
    <ext:Button ID="Button1" runat="server" Icon="Report" Text="Generate">
      <DirectEvents>
        <Click OnEvent="Report_OnGenerate" Before="return isValidForm(#{frmExpireBatch});">
    ...
    <ext:Button ID="Button2" runat="server" Icon="Reload" Text="Bersihkan">
      <Listeners>
        <Click Handler="initializePanel(#{frmExpireBatch}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
      </Listeners>
    </ext:Button>
  </Buttons>
  <Listeners>
    <AfterRender Handler="initializePanel(#{frmExpireBatch}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
  </Listeners>
</ext:Panel>
