...
public partial class reporting_Inventory_KartuBarangNasional : System.Web.UI.UserControl
{
  ...
  protected void Report_OnGenerate(object sender, DirectEventArgs e)
  {
    ...
    if (rdBatch.Checked)
    {
      rptParse.ReportingID = "10004-a";
    }
    else
    {
      rptParse.ReportingID = "10004-b";
    }
    ...
    //  {
    //    DataType = typeof(string).FullName,
    //    ParameterName = string.Format("({{command.c_iteno}} IN [{0}])", string.Join(",", lstData.ToArray())),
    //    IsReportDirectValue = true
    //  });
    ...
    else
    {
      if (rptResult.IsSuccess)
      {
        ...
        {
          string rptName = string.Concat("Inventory_KartuBarangNasional_", pag.Nip, ".", rptResult.Extension);
