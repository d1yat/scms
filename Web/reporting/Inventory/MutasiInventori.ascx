<%@ Control Language="C#" AutoEventWireup="true" CodeFile="MutasiInventori.ascx.cs"
  Inherits="reporting_Inventory_MutasiInventori" %>
<ext:Panel ID="Panel1" runat="server">
  <Items>
    <ext:Hidden ID="hidWndDown" runat="server" />
    <ext:FormPanel ID="frmMutasiInv" runat="server" Padding="5" Frame="True" Layout="Form">
      <Items>
        ...
        <ext:RadioGroup ID="rdgTipe" runat="server" ColumnsNumber="1" FieldLabel="Tipe Laporan"
              AllowBlank="false">
              <Items>
                <ext:Radio ID="rdRekap" runat="server" BoxLabel="Rekap" />
                <ext:Radio ID="rdDetail" runat="server" BoxLabel="Detail" Checked="true" />
              </Items>
        </ext:RadioGroup>
  ...
  <Buttons>
    <ext:Button ID="Button1" runat="server" Icon="Report" Text="Generate">
      <DirectEvents>
        <Click OnEvent="Report_OnGenerate" Before="return isValidForm(#{frmMutasiInv});">
    ...
    <ext:Button ID="Button2" runat="server" Icon="Reload" Text="Bersihkan">
      <Listeners>
        <Click Handler="initializePanel(#{frmMutasiInv}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
      </Listeners>
    </ext:Button>
  </Buttons>
  <Listeners>
    <AfterRender Handler="initializePanel(#{frmMutasiInv}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
  </Listeners>
</ext:Panel>
