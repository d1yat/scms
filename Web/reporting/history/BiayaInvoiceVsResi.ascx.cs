...
public partial class reporting_history_BiayaInvoiceVsResi : System.Web.UI.UserControl
{
  ...
  protected void Report_OnGenerate(object sender, DirectEventArgs e)
  {
    ...
    if (!string.IsNullOrEmpty(txIENo1.Text))
    {
        if (string.IsNullOrEmpty(txIENo2.Text))
        {
            ...
            {
                DataType = typeof(string).FullName,
                ParameterName = "temp_invoicevsresi.c_ieno",
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    DataType = typeof(string).FullName,
                    ParameterName = string.Format("({{temp_invoicevsresi.c_ieno}} IN ('{0}' TO '{1}'))", txIENo1.Text, txIENo2.Text),
    ...
    if (!string.IsNullOrEmpty(txNoResi1.Text))
    {
        if (string.IsNullOrEmpty(txNoResi2.Text))
        {
            ...
            {
                DataType = typeof(string).FullName,
                ParameterName = "temp_invoicevsresi.c_resi",
                ParameterValue = (string.IsNullOrEmpty(txNoResi1.Text) ? string.Empty : txNoResi1.Text)
            });
        }
        else
        {
            if (txIENo1.Text.CompareTo(txNoResi2.Text) >= 0)
            {
                lstRptParam.Add(new ReportParameter()
                {
                    DataType = typeof(string).FullName,
            ...
            else
            {
                ...
                {
                    DataType = typeof(string).FullName,
                    ParameterName = string.Format("({{temp_invoicevsresi.c_resi}} IN ('{0}' TO '{1}'))", txNoResi1.Text, txNoResi2.Text),
                    IsReportDirectValue = true
                });
            }
        }
