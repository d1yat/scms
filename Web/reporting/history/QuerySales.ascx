<%@ Control Language="C#" AutoEventWireup="true" CodeFile="QuerySales.ascx.cs" Inherits="reporting_history_QuerySales" %>
<ext:Panel runat="server">
  <Items>
    <ext:Hidden ID="hidWndDown" runat="server" />
    <ext:FormPanel ID="frmReportQuerySales" runat="server" Padding="5" Frame="True" Layout="Form">
      <Items>
        ...
        <ext:SelectBox ID="cbJenisReport" runat="server" FieldLabel="Output" SelectedIndex="0"
          AllowBlank="false">
          <Items>
            <ext:ListItem Value="sum" Text="Summary" />
            <ext:ListItem Value="cab" Text="Per Cabang" />
            <ext:ListItem Value="no" Text="Per No Faktur" />
          </Items>
        </ext:SelectBox>
  ...
  <Buttons>
    <ext:Button runat="server" Icon="ReportGo" Text="Generate">
      <DirectEvents>
        <Click OnEvent="Report_OnGenerate" Before="return isValidForm(#{frmReportQuerySales});">
    ...
    <ext:Button runat="server" Icon="Reload" Text="Bersihkan">
      <Listeners>
        <Click Handler="initializePanel(#{frmReportQuerySales}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
      </Listeners>
    </ext:Button>
  </Buttons>
  <Listeners>
    <AfterRender Handler="initializePanel(#{frmReportQuerySales}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
  </Listeners>
</ext:Panel>
