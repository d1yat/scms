...
public partial class reporting_history_QueryPenjualan : System.Web.UI.UserControl
{
  ...
  protected void Report_OnGenerate(object sender, DirectEventArgs e)
  {
    ...
    if (chkJual.Checked == true)
    {
      rptParse.ReportingID = "20012-1";
    }
    else
    {
      rptParse.ReportingID = "20012-2";
    }
    ...
    if (rdHeader.Checked == true)
    {
        ...
        if (chkJual.Checked == true)
        {
            ...
            else if (chkFltFJPri.Checked)
            {
                tipereport = "PPrinFJ";
            }

            else if (chkFltFJAms.Checked)
            {
                tipereport = "PAmsFJ";
            }
        ...
        else // Retur Jual
        {
            if (Scms.Web.Common.Functional.DateParser(txPeriode1.RawText.Trim(), "d-M-yyyy", out date1))
            {
                ...
                if ((!date1.Equals(DateTime.MinValue)) && (!date1.Equals(DateTime.MinValue)) && (!date1.Equals(date2)))
                {
                    ...
                    {
                        DataType = typeof(string).FullName,
                        ParameterName = string.Format("({{LG_fjrh.d_fjdate}} IN {0} to {1})", Functional.CrystalReportDateString(date1), Functional.CrystalReportDateString(date2)),
                        IsReportDirectValue = true
                    });
                }
                else if (!date1.Equals(DateTime.MinValue))
                {
                    ...
                    {
                        DataType = typeof(string).FullName,
                        ParameterName = "LG_fjrh.d_fjdate",
                        ParameterValue = date1.ToString("yyyy-MM-dd")
                    });
                }
            }
    ...
    else if (rdDetail.Checked == true)
    {
        ...
        if (chkJual.Checked)
        {
            tipereport = "Penjualan";
        }
        else if (chkRetur.Checked)
        {
            tipereport = "ReturPenjualan";
        }
    ...
    else
    {
      if (rptResult.IsSuccess)
      {
        ...
        {
          string rptName = string.Concat("History_Query_Penjualan_", pag.Nip, ".", rptResult.Extension);
