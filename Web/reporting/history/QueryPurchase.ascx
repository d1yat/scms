<%@ Control Language="C#" AutoEventWireup="true" CodeFile="QueryPurchase.ascx.cs"
  Inherits="reporting_history_QueryPurchase" %>
<ext:Panel runat="server">
  <Items>
    <ext:Hidden ID="hidWndDown" runat="server" />
    <ext:FormPanel ID="frmReportQueryPurchase" runat="server" Padding="5" Frame="True"
      Layout="Form">
      <Items>
          ...
          <Store>
            <ext:Store runat="server">
              ...
              <BaseParams>
                ...
                <ext:Parameter Name="parameters" Value="[['@contains.c_iteno.Contains(@0) || @contains.v_itnam.Contains(@0)', paramTextGetter(#{cbItems}), '']]"
                  Mode="Raw" />
        ...
        <ext:Checkbox ID="chkIsRetur" runat="server" FieldLabel="Retur" />
        <ext:RadioGroup ID="cbgJenisReport" runat="server" ColumnsNumber="1" FieldLabel="Jenis">
          <Items>
            <ext:Radio ID="rdSum" runat="server" BoxLabel="Summary" Checked="true" Tag="sum" />
            <ext:Radio ID="rdCab" runat="server" BoxLabel="Per Supplier" Tag="cab" />
            <ext:Radio ID="rdNo" runat="server" BoxLabel="Per No Faktur" Tag="no" />
          </Items>
        </ext:RadioGroup>
  ...
  <Buttons>
    <ext:Button runat="server" Icon="ReportGo" Text="Generate">
      <DirectEvents>
        <Click OnEvent="Report_OnGenerate" Before="return isValidForm(#{frmReportQueryPurchase});">
    ...
    <ext:Button runat="server" Icon="Reload" Text="Bersihkan">
      <Listeners>
        <Click Handler="initializePanel(#{frmReportQueryPurchase}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
      </Listeners>
    </ext:Button>
  </Buttons>
  <Listeners>
    <AfterRender Handler="initializePanel(#{frmReportQueryPurchase}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
  </Listeners>
</ext:Panel>
