...
public partial class reporting_history_SuratTandaTerima : System.Web.UI.UserControl
{
  ...
  protected void Report_OnGenerate(object sender, DirectEventArgs e)
  {
    ...
    rptParse.ReportingID = "20006";

    isAsync = chkAsync.Checked;

    tmp = txMemoSTT1.Text.Trim();
    tmp1 = txMemoSTT2.Text.Trim();
    ...
    {
      DataType = typeof(string).FullName,
      ParameterName = "Temp_LGHistorySTT.c_user",
      ParameterValue = pag.Nip
    });
    ...
    if (!string.IsNullOrEmpty(cbGudang.SelectedItem.Value))
    {
      ...
      {
        DataType = typeof(string).FullName,
        ParameterName = "Temp_LGHistorySTT.c_gdg",
    ...
    if (!string.IsNullOrEmpty(cbItems.SelectedItem.Value))
    {
      ...
      {
        DataType = typeof(string).FullName,
        ParameterName = "Temp_LGHistorySTT.c_iteno",
    ...
    //  {
    //    DataType = typeof(string).FullName,
    //    ParameterName = string.Format("({{Temp_LGHistorySTT.c_iteno}} IN [{0}])", string.Join(",", lstData.ToArray())),
    ...
    if (tmp.Length > 0)
    {
      ...
      {
        DataType = typeof(string).FullName,
        ParameterName = string.Format("({{Temp_LGHistorySTT.c_memo}} Like '*{0}*')", tmp),
        IsReportDirectValue = true
      });
    }
