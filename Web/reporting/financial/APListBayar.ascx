<%@ Control Language="C#" AutoEventWireup="true" CodeFile="APListBayar.ascx.cs" 
Inherits="reporting_financial_APListBayar" %>

<ext:Panel runat="server">
  <Items>
    <ext:Hidden ID="hidWndDown" runat="server" />
    <ext:FormPanel ID="frmReportAPListBayar" runat="server" Padding="5" Frame="True" 
      Layout="Form">
      <Items>
        ...
        <ext:RadioGroup ID="rdgTipe" runat="server" ColumnsNumber="1" FieldLabel="Tipe Laporan" AllowBlank="false">
          <Items>
            <ext:Radio ID="rdA" runat="server" BoxLabel="Type A" Checked="true" InputValue="rdA" />
            <ext:Radio ID="rdB" runat="server" BoxLabel="Type B" InputValue="rdB" />
          </Items>
        </ext:RadioGroup>
  ...
  <Buttons>
    <ext:Button runat="server" Icon="ReportGo" Text="Generate">
      <DirectEvents>
        <Click OnEvent="Report_OnGenerate" Before="return isValidForm(#{frmReportAPListBayar});">
    ...
    <ext:Button runat="server" Icon="Reload" Text="Bersihkan">
      <Listeners>
        <Click Handler="initializePanel(#{frmReportAPListBayar}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
      </Listeners>
    </ext:Button>
  </Buttons>
  <Listeners>
    <AfterRender Handler="initializePanel(#{frmReportAPListBayar}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
  </Listeners>
</ext:Panel>
