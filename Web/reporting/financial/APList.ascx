<%@ Control Language="C#" AutoEventWireup="true" CodeFile="APList.ascx.cs" 
Inherits="reporting_financial_APList" %>

<ext:Panel runat="server">
  <Items>
    <ext:Hidden ID="hidWndDown" runat="server" />
    <ext:FormPanel ID="frmReportAPList" runat="server" Padding="5" Frame="True" 
      Layout="Form">
      <Items>
        ...
        <ext:RadioGroup ID="rdgTipe" runat="server" ColumnsNumber="1" FieldLabel="Tipe Laporan">
          <Items>
            <ext:Radio ID="rdSummary" runat="server" BoxLabel="Summary" Checked="true" InputValue="summary" />
        ...
        <ext:RadioGroup ID="rdgTipeFak" runat="server" ColumnsNumber="1" FieldLabel="Tipe Faktur">
          <Items>
            <ext:Radio ID="rdFak" runat="server" BoxLabel="Faktur" Checked="true" InputValue="fak" />
            <ext:Radio ID="rdFakRet" runat="server" BoxLabel="Faktur Retur" InputValue="fakRet" />
          </Items>
        </ext:RadioGroup>
  ...
  <Buttons>
    <ext:Button runat="server" Icon="ReportGo" Text="Generate">
      <DirectEvents>
        <Click OnEvent="Report_OnGenerate" Before="return isValidForm(#{frmReportAPList});">
    ...
    <ext:Button runat="server" Icon="Reload" Text="Bersihkan">
      <Listeners>
        <Click Handler="initializePanel(#{frmReportAPList}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
      </Listeners>
    </ext:Button>
  </Buttons>
  <Listeners>
    <AfterRender Handler="initializePanel(#{frmReportAPList}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
  </Listeners>
</ext:Panel>
