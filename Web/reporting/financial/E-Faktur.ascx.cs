...
public partial class reporting_history_EFaktur : System.Web.UI.UserControl
{
  ...
  protected void Report_OnGenerate(object sender, DirectEventArgs e)
  {
    ...
    if (rdFJ.Checked)   //faktur jual
    {
        rptParse.ReportingID = "20411";
        ...
        if (Scms.Web.Common.Functional.DateParser(txPeriode1.RawText.Trim(), "d-M-yyyy", out date1))
        {
            ...
            if ((!date1.Equals(DateTime.MinValue)) && (!date1.Equals(DateTime.MinValue)) && (!date1.Equals(date2)))
            {
                lstRptParam.Add(new ReportParameter()
                {
                    ...
                    ParameterName = string.Format("({{LG_FJH.d_fjdate}} IN {0} to {1})", Functional.CrystalReportDateString(date1), Functional.CrystalReportDateString(date2)),
        ...
        if (!string.IsNullOrEmpty(txFakturno1.Text.Trim()))
        {
            if (!string.IsNullOrEmpty(txFakturno2.Text.Trim()))
            {
                ...
                {
                    DataType = typeof(string).FullName,
                    ParameterName = string.Format("({{LG_FJH.c_fjno}} IN '{0}' to '{1}')", txFakturno1.Text.Trim(), txFakturno2.Text.Trim()),
                    IsReportDirectValue = true
                });
            }
            else
            {
                ...
                {
                    DataType = typeof(string).FullName,
                    ParameterName = string.Format("({{LG_FJH.c_fjno}} = '{0}')", txFakturno1.Text.Trim()),
                    IsReportDirectValue = true
                });
            }
        }
        ...
        {
            DataType = typeof(string).FullName,
            ParameterName = string.Format("({{lg_cusmas.l_cabang}} = {0})", "false"),
            IsReportDirectValue = true
        });
    }
    else    //faktur manual
    {
        rptParse.ReportingID = "20413";
        ...
        if (!string.IsNullOrEmpty(txFakturno1.Text.Trim()))
        {
            if (!string.IsNullOrEmpty(txFakturno2.Text.Trim()))
            {
                ...
                {
                    DataType = typeof(string).FullName,
                    ParameterName = string.Format("({{lg_fm.c_taxno}} IN '{0}' to '{1}')", txFakturno1.Text.Trim(), txFakturno2.Text.Trim()),
                    IsReportDirectValue = true
                });
            }
            else
            {
                ...
                {
                    DataType = typeof(string).FullName,
                    ParameterName = string.Format("({{lg_fm.c_taxno}} = '{0}')", txFakturno1.Text.Trim()),
                    IsReportDirectValue = true
                });
            }
        }
    ...
    else
    {
      if (rptResult.IsSuccess)
      {
        ...
        {
          string rptName = string.Concat("E-Faktur", pag.Nip, ".", rptResult.Extension);
