<%@ Control Language="C#" AutoEventWireup="true" CodeFile="HPPDivAMSExtDisc.ascx.cs" Inherits="reporting_financial_HPPDivAMSExtDisc" %>
<ext:Panel runat="server">
  <Items>
    <ext:Hidden ID="hidWndDown" runat="server" />
    <ext:FormPanel ID="frmReportHPPDivAMSExtDisc" runat="server" Padding="5" Frame="True" Layout="Form">
      <Items>
  ...
  <Buttons>
    <ext:Button runat="server" Icon="ReportGo" Text="Generate">
      <DirectEvents>
        <Click OnEvent="Report_OnGenerate" Before="return isValidForm(#{frmReportHPPDivAMSExtDisc});">
    ...
    <ext:Button runat="server" Icon="Reload" Text="Bersihkan">
      <Listeners>
        <Click Handler="initializePanel(#{frmReportHPPDivAMSExtDisc}, #{chkAsync});" />
      </Listeners>
    </ext:Button>
  </Buttons>
  <Listeners>
    <AfterRender Handler="initializePanel(#{frmReportHPPDivAMSExtDisc}, #{chkAsync});" />
  </Listeners>
</ext:Panel>
