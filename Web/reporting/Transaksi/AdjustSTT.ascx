<%@ Control Language="C#" AutoEventWireup="true" CodeFile="AdjustSTT.ascx.cs" 
Inherits="reporting_Transaksi_AdjustSTT" %>

<ext:Panel runat="server">
  <Items>
    <ext:Hidden ID="hidWndDown" runat="server" />
    <ext:FormPanel ID="frmReportAdjSTT" runat="server" Padding="5" Frame="True" 
      Layout="Form">
      <Items>
        ...
        <ext:RadioGroup ID="rdgTipeSTT" runat="server" ColumnsNumber="1" FieldLabel="Tipe Adjust">
          <Items>
            <ext:Radio ID="rdDonasi" runat="server" BoxLabel="Donasi" Tag="donasi" Checked="true" />
            <ext:Radio ID="rdSample" runat="server" BoxLabel="Sample" Tag="sample" />
          </Items>
        </ext:RadioGroup>
  ...
  <Buttons>
    <ext:Button runat="server" Icon="ReportGo" Text="Generate">
      <DirectEvents>
        <Click OnEvent="Report_OnGenerate" Before="return isValidForm(#{frmReportAdjSTT});">
    ...
    <ext:Button runat="server" Icon="Reload" Text="Bersihkan">
      <Listeners>
        <Click Handler="initializePanel(#{frmReportAdjSTT}, #{chkAsync});" />
      </Listeners>
    </ext:Button>
  </Buttons>
  <Listeners>
    <AfterRender Handler="initializePanel(#{frmReportAdjSTT}, #{chkAsync});" />
  </Listeners>
</ext:Panel>
