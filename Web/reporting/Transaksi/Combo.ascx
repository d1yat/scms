<%@ Control Language="C#" AutoEventWireup="true" CodeFile="Combo.ascx.cs" 
Inherits="reporting_Transaksi_Combo" %>
<ext:Panel runat="server">
  <Items>
    <ext:Hidden ID="hidWndDown" runat="server" />
    
    <ext:FormPanel ID="frmReportCombo" runat="server" Padding="5" Frame="True"
      Layout="Form">
      <Items>
        <ext:CompositeField runat="server" FieldLabel="Jarak Memo Combo">
          <Items>
            <ext:TextField ID="txMemoCombo1" runat="server" AllowBlank="True" />
            <ext:Label runat="server" Text="-" />
            <ext:TextField ID="txMemoCombo2" runat="server" AllowBlank="True" />
          </Items>
        </ext:CompositeField>
  ...
  <Buttons>
    <ext:Button  runat="server" Icon="ReportGo" Text="Generate">
      <DirectEvents>
        <Click OnEvent="Report_OnGenerate" Before="return isValidForm(#{frmReportCombo});">
    ...
    <ext:Button runat="server" Icon="Reload" Text="Bersihkan">
      <Listeners>
        <Click Handler="initializePanel(#{frmReportCombo}, #{chkAsync});" />
      </Listeners>
    </ext:Button>
  </Buttons>
  <Listeners>
    <AfterRender Handler="initializePanel(#{frmReportCombo}, #{chkAsync});" />
  </Listeners>
</ext:Panel>
