...
public partial class reporting_Transaksi_AdjustPurchaseOrder : System.Web.UI.UserControl
{
  ...
  protected void Report_OnGenerate(object sender, DirectEventArgs e)
  {
    ...
    if (txADJ1.Text.Length > 0)
    {
      if (txADJ2.Text.Length > 0)
      {
        ...
        {
          DataType = typeof(string).FullName,
          ParameterName = string.Format("({{lg_adjpoh.c_adjno}} IN ['{0}','{1}'])", txADJ1.Text, txADJ2.Text),
      ...
      else
      {
        ...
        {
          DataType = typeof(string).FullName,
          ParameterName = string.Format("({{lg_adjpoh.c_adjno}} >= '{0}')", txADJ1.Text),
          IsReportDirectValue = true
        });
      }
    }
    else if (txADJ2.Text.Length > 0)
    {
      ...
      {
        DataType = typeof(string).FullName,
        ParameterName = string.Format("({{lg_adjpoh.c_adjno}} <= '{0}')", txADJ2.Text),
        IsReportDirectValue = true
      });
    }
    ...
    if (!string.IsNullOrEmpty(cbItems.SelectedItem.Value))
    {
      ...
      {
        DataType = typeof(string).FullName,
        ParameterName = "lg_adjpod.c_iteno",
    ...
    //  {
    //    DataType = typeof(string).FullName,
    //    ParameterName = string.Format("({{LG_adjustTransH.c_cussup}} IN [{0}])", string.Join(",", lstData.ToArray())),
    //    IsReportDirectValue = true
    //  });
    ...
    //  {
    //    DataType = typeof(string).FullName,
    //    ParameterName = "LG_adjustTransH.c_cussup",
    ...
    //{
    //  DataType = typeof(string).FullName,
    //  ParameterName = "({lg_adjpoh.c_type} = '08')",
    //  IsReportDirectValue = true
    //});
    ...
    else
    {
      if (rptResult.IsSuccess)
      {
        ...
        {
          string rptName = string.Concat("Transaksi_Adjust_PO_", pag.Nip, ".", rptResult.Extension);
