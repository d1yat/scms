<%@ Control Language="C#" AutoEventWireup="true" CodeFile="AdjustPurchaseOrder.ascx.cs" 
Inherits="reporting_Transaksi_AdjustPurchaseOrder" %>

<ext:Panel runat="server">
  <Items>
    <ext:Hidden ID="hidWndDown" runat="server" />
    <ext:FormPanel ID="frmReportAdjPO" runat="server" Padding="5" Frame="True" 
      Layout="Form">
      <Items>
  ...
  <Buttons>
    <ext:Button runat="server" Icon="ReportGo" Text="Generate">
      <DirectEvents>
        <Click OnEvent="Report_OnGenerate" Before="return isValidForm(#{frmReportAdjPO});">
    ...
    <ext:Button runat="server" Icon="Reload" Text="Bersihkan">
      <Listeners>
        <Click Handler="initializePanel(#{frmReportAdjPO}, #{chkAsync});" />
      </Listeners>
    </ext:Button>
  </Buttons>
  <Listeners>
    <AfterRender Handler="initializePanel(#{frmReportAdjPO}, #{chkAsync});" />
  </Listeners>
</ext:Panel>
