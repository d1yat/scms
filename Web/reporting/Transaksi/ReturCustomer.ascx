<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ReturCustomer.ascx.cs" 
Inherits="reporting_Transaksi_ReturCustomer" %>

<ext:Panel ID="Panel1" runat="server">
  <Items>
    <ext:Hidden ID="hidWndDown" runat="server" />
    
    <ext:FormPanel ID="frmReportRC" runat="server" Padding="5" Frame="True" 
      Layout="Form">
      <Items>
        ...
        <ext:CompositeField runat="server" FieldLabel="Batasan RC">
          <Items>
            <ext:TextField ID="txRC1" runat="server" AllowBlank="true" />
  ...
  <Buttons>
    <ext:Button ID="Button1" runat="server" Icon="ReportGo" Text="Generate">
      <DirectEvents>
        <Click OnEvent="Report_OnGenerate" Before="return isValidForm(#{frmReportRC});">
    ...
    <ext:Button ID="Button2" runat="server" Icon="Reload" Text="Bersihkan">
      <Listeners>
        <Click Handler="initializePanel(#{frmReportRC}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
      </Listeners>
    </ext:Button>
  </Buttons>
  <Listeners>
    <AfterRender Handler="initializePanel(#{frmReportRC}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
  </Listeners>
</ext:Panel>
