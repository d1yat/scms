...
public partial class reporting_Transaksi_PackingList : System.Web.UI.UserControl
{
  ...
  protected void Report_OnGenerate(object sender, DirectEventArgs e)
  {
    ...
    rptParse.ReportingID = "21007";

    isAsync = chkAsync.Checked;

    tmp = txPL1.Text.Trim();
    tmp1 = txPL2.Text.Trim();
    ...
    if (Scms.Web.Common.Functional.DateParser(txPeriode1.RawText.Trim(), "d-M-yyyy", out date1))
    {
      ...
      if ((!date1.Equals(DateTime.MinValue)) && (!date1.Equals(DateTime.MinValue)) && (!date1.Equals(date2)))
      {
        ...
        {
          DataType = typeof(string).FullName,
          ParameterName = string.Format("({{LG_PLH.d_pldate}} IN {0} to {1})", Functional.CrystalReportDateString(date1), Functional.CrystalReportDateString(date2)),
    ...
    if (txPL1.Text.Length > 0)
    {
      if (txPL2.Text.Length > 0)
      {
        lstRptParam.Add(new ReportParameter()
        {
          DataType = typeof(string).FullName,
          ParameterName = string.Format("({{LG_PLH.c_plno}} IN ['{0}','{1}'])", txPL1.Text, txPL2.Text),
          IsReportDirectValue = true
        });
      }
      else
      {
        ...
        {
          DataType = typeof(string).FullName,
          ParameterName = string.Format("({{LG_PLH.c_plno}} >= '{0}')", txPL1.Text),
          IsReportDirectValue = true
        });
      }
    }
    else if (txPL2.Text.Length > 0)
    {
      lstRptParam.Add(new ReportParameter()
      {
        DataType = typeof(string).FullName,
        ParameterName = string.Format("({{LG_PLH.c_plno}} <= '{0}')", txPL2.Text),
        IsReportDirectValue = true
      });
    }
    ...
    //  {
    //    DataType = typeof(string).FullName,
    //    ParameterName = string.Format("({{LG_PLH.c_nosup}} IN [{0}])", string.Join(",", lstData.ToArray())),
    //    IsReportDirectValue = true
    //  });
    ...
    else
    {
      if (rptResult.IsSuccess)
      {
        ...
        {
          string rptName = string.Concat("Transaksi_PL_", pag.Nip, ".", rptResult.Extension);
