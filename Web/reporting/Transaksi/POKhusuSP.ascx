<%@ Control Language="C#" AutoEventWireup="true" CodeFile="POKhusuSP.ascx.cs" Inherits="reporting_Transaksi_POKhusuSP" %>
<ext:Panel runat="server">
  ...
  <Items>
    <ext:FormPanel ID="frmReportPOK" runat="server" Padding="5" Frame="True" Layout="Form">
      <Items>
        ...
        <ext:ComboBox ID="cbCustomer" runat="server" DisplayField="v_cunam" ValueField="c_cusno"
          Width="500" ItemSelector="tr.search-item" PageSize="10" ListWidth="500"
  ...
  <Buttons>
    <ext:Button runat="server" Icon="ReportGo" Text="Generate">
      <DirectEvents>
        <Click OnEvent="Report_OnGenerate" Before="return isValidForm(#{frmReportPOK});">
    ...
    <ext:Button runat="server" Icon="Reload" Text="Bersihkan">
      <Listeners>
        <Click Handler="initializePanel(#{frmReportPOK}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
      </Listeners>
    </ext:Button>
  </Buttons>
  <Listeners>
    <AfterRender Handler="initializePanel(#{frmReportPOK}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
  </Listeners>
</ext:Panel>
