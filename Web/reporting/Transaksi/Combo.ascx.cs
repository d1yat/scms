...
public partial class reporting_Transaksi_Combo : System.Web.UI.UserControl
{
  ...
  protected void Report_OnGenerate(object sender, DirectEventArgs e)
  {
      ...
      if (!string.IsNullOrEmpty(cbItems.SelectedItem.Value))
    {
        ...
        {
            DataType = typeof(string).FullName,
            ParameterName = "lg_combod1.c_iteno",
      ...
      //{
      //  DataType = typeof(string).FullName,
      //  ParameterName = string.Format("({{lg_combod1.c_iteno}} IN [{0}])", string.Join(",", lstData.ToArray())),
    ...
    if (txMemoCombo1.Text.Length > 0)
    {
        if (txMemoCombo2.Text.Length > 0)
        {
            lstRptParam.Add(new ReportParameter()
            {
                DataType = typeof(string).FullName,
                ParameterName = string.Format("({{lg_comboh.c_combono}} IN ['{0}','{1}'])", txMemoCombo1.Text.Trim(), txMemoCombo2.Text.Trim()),
                IsReportDirectValue = true
            });
        }
        else
        {
            ...
            {
                DataType = typeof(string).FullName,
                ParameterName = string.Format("({{lg_comboh.c_combono}} >= '{0}')", txMemoCombo1.Text.Trim()),
                IsReportDirectValue = true
            });
        }
    }
    else if (txMemoCombo2.Text.Length > 0)
    {
        lstRptParam.Add(new ReportParameter()
        {
            DataType = typeof(string).FullName,
            ParameterName = string.Format("({{lg_comboh.c_combono}} <= '{0}')", txMemoCombo2.Text.Trim()),
            IsReportDirectValue = true
        });
    }
    #endregion
    ...
    else
    {
      if (rptResult.IsSuccess)
      {
        ...
        {
          string rptName = string.Concat("List_Transaksi_Combo_", pag.Nip, ".", rptResult.Extension);
