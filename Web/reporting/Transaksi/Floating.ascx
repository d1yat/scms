<%@ Control Language="C#" AutoEventWireup="true" CodeFile="Floating.ascx.cs" 
Inherits="reporting_Transaksi_Floating" %>

<ext:Panel runat="server">
  <Items>
    <ext:Hidden ID="hidWndDown" runat="server" />
    <ext:FormPanel ID="frmReportFloating" runat="server" Padding="5" Frame="True" 
      Layout="Form">
      <Items>
        ...
        <ext:CompositeField runat="server" FieldLabel="Batasan RN">
          <Items>
            <ext:TextField ID="txFloat1" runat="server" AllowBlank="true" />
            <ext:Label ID="Label2" runat="server" Text="-" AllowBlank="true" />
            <ext:TextField ID="txFloat2" runat="server" />
          </Items>
        </ext:CompositeField>
  ...
  <Buttons>
    <ext:Button runat="server" Icon="ReportGo" Text="Generate">
      <DirectEvents>
        <Click OnEvent="Report_OnGenerate" Before="return isValidForm(#{frmReportFloating});">
    ...
    <ext:Button runat="server" Icon="Reload" Text="Bersihkan">
      <Listeners>
        <Click Handler="initializePanel(#{frmReportFloating}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
      </Listeners>
    </ext:Button>
  </Buttons>
  <Listeners>
    <AfterRender Handler="initializePanel(#{frmReportFloating}, #{chkAsync}, #{txPeriode1}, #{txPeriode2});" />
  </Listeners>
</ext:Panel>
