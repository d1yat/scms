...
public partial class reporting_Transaksi_PurchaseOrder : System.Web.UI.UserControl
{
  ...
  protected void Report_OnGenerate(object sender, DirectEventArgs e)
  {
    ...
    {
      rptParse.ReportingID = "21001-a";
    }
    if (rdPO.Checked == true)
    {
      rptParse.ReportingID = "21001-b";
    }

    isAsync = chkAsync.Checked;


    tmp = txNoPO1.Text.Trim();
    tmp1 = txNoPO2.Text.Trim();
    ...
    if (Scms.Web.Common.Functional.DateParser(txPeriode1.RawText.Trim(), "d-M-yyyy", out date1))
    {
      ...
      if ((!date1.Equals(DateTime.MinValue)) && (!date1.Equals(DateTime.MinValue)) && (!date1.Equals(date2)))
      {
        lstRptParam.Add(new ReportParameter()
        {
          ...
          ParameterName = string.Format("({{lg_poh.d_podate}} IN '{0}' to '{1}')", date1.ToString("yyyy-MM-dd"), date2.ToString("yyyy-MM-dd")),
    ...
    tmp = Functional.GetCheckedRadioData(rdgTipePO);
    ...
    if (!string.IsNullOrEmpty(tmp))
    {
      if (tmp.Contains("oto"))
      {
        ...
        {
          DataType = typeof(string).FullName,
          ParameterName = "({lg_orh.c_type} = '01')",
          IsReportDirectValue = true
        });
      }
      if (tmp.Contains("khs"))
      {
        ...
        {
          DataType = typeof(string).FullName,
          ParameterName = "({lg_orh.c_type} = '02')",
          IsReportDirectValue = true
        });
      }
      if (tmp.Contains("tgh"))
      {
        ...
        {
          DataType = typeof(string).FullName,
          ParameterName = "({lg_orh.c_type} = '03')",
          IsReportDirectValue = true
        });
      }
      if (tmp.Contains("akr"))
      {
        ...
        {
          DataType = typeof(string).FullName,
          ParameterName = "({lg_orh.c_type} = '04')",
          IsReportDirectValue = true
        });
      }
      if (tmp.Contains("man"))
      {
        ...
        {
          DataType = typeof(string).FullName,
          ParameterName = "({lg_orh.c_type} = '05')",
          IsReportDirectValue = true
        });
      }
      //lstData.Clear();
    ...
    if (txNoPO1.Text.Length > 0)
    {
      if (txNoPO2.Text.Length > 0)
      {
        lstRptParam.Add(new ReportParameter()
        {
          DataType = typeof(string).FullName,
          ParameterName = string.Format("({{lg_poh.c_pono}} IN ['{0}','{1}'])", txNoPO1.Text.Trim(), txNoPO2.Text.Trim()),
          IsReportDirectValue = true
        });
      }
      else
      {
        ...
        {
          DataType = typeof(string).FullName,
          ParameterName = string.Format("({{lg_poh.c_pono}} >= '{0}')", txNoPO1.Text.Trim()),
          IsReportDirectValue = true
        });
      }
    }
    else if (txNoPO2.Text.Length > 0)
    {
      lstRptParam.Add(new ReportParameter()
      {
        DataType = typeof(string).FullName,
        ParameterName = string.Format("({{lg_poh.c_pono}} <= '{0}')", txNoPO2.Text.Trim()),
        IsReportDirectValue = true
      });
    }
    ...
    else
    {
      if (rptResult.IsSuccess)
      {
        ...
        {
          string rptName = string.Concat("Report_PO", pag.Nip, ".", rptResult.Extension);
