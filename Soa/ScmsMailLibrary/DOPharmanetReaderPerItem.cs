...
{
    public class DOPharmanetReaderPerItem
    {
        ...
        private string plphar;

        public DOPharmanetReaderPerItem(ScmsSoaLibrary.Commons.Config config)
        {
            this.config = config;
            ...
            try
            {
                ...
                if (dataset.Tables.Contains(DEFAULT_NAME_TABEL_HEADER))
                {
                    ...
                    for (nLoop = 0, nLen = table.Rows.Count; nLoop < nLen; nLoop++)
                    {
                        ...
                        //if (kodePrinsipal == "00165" || kodePrinsipal == "00001")
                        if (kodePrinsipal == "00165")
                        {
                            nipEntry = "SPPHAR2";
                        }
                        else
                        ...
                        try
                        {
                            ...
                            spID = Commons2.GenerateNumbering<LG_SPH>(dbSpPo, "SP", '3', "07", dateToday, "c_spno");
                            ...
                            poID = Commons2.GenerateNumbering<LG_POH>(dbSpPo, "PO", '3', "02", dateToday, "c_pono");
                            ...
                            {
                                c_nosup = kodePrinsipal,
                                c_dono = row.GetValue<string>("C_NODO", string.Empty).Trim(),
                                d_dodate = dateToday,
                                d_fjno = row.GetValue<string>("C_EXNOINV", string.Empty),
                                d_fjdate = date,
                                l_status = true,
                                ...
                                c_taxno = row.GetValue<string>("C_NOTAX", string.Empty),
                                d_entry = dateNow,
                                c_plphar = row.GetValue<string>("N_NOPL", string.Empty).Trim(),
                                c_po_outlet = sPoOutlet
                            };
                                    ...
                                    for (nLoopC = 0, nLenC = rowCols.Length; nLoopC < nLenC; nLoopC++)
                                    {
                                        ...
                                        if (string.IsNullOrEmpty(kode_item))
                                        {
                                            kode_item = (from q in db.FA_MasItms
                                                         where q.c_itenopri == noPric && q.l_aktif == true
                                            ...
                                            if (sup != kodePrinsipal)
                                            {
                                                kode_item = (from q in db.FA_MasItms
                                                             where q.c_itenopri == noPric
                                                                && q.c_nosup == "00165" && q.l_aktif == true
                                                             select q.c_iteno).Take(1).SingleOrDefault();
                                            }
                                        }
                                        //}
                    ...
                    for (nLoopRh = 0, nLenC = lstDOPH.Count; nLoopRh < nLenC; nLoopRh++)
                    {
                        ...
                        plphar = lstDOPH[nLoopRh].c_po_outlet;
                       
                        rnIDTemp = (from q in db.LG_RNHs
                                    where q.c_po_outlet == lstDOPH[nLoopRh].c_po_outlet
                                      && q.c_dono == lstDOPH[nLoopRh].c_dono
                                      && q.c_from == "00165"
                                    select q.c_rnno).SingleOrDefault();
                    ...
                    for (nLoopPh = 0, nLoop = lstDoPharLink.Count; nLoopPh < nLoop; nLoopPh++)
                    {
                        ...
                        plphar = lstDoPharLink[nLoopPh].OutletPO;
                        if (plphar == "01185404-2")
                        {
                            var tes = "";
                        }
                        ...
                        checkDoh = (from q in db.LG_DOHs
                                    where q.c_po_outlet == lstDoPharLink[nLoopPh].OutletPO
                                        && q.c_rnno == null // && q.c_dono.Contains("DO1609")
                            ...
                            for (nLoopPd = 0, nLen = lstRND2.Count; nLoopPd < nLen; nLoopPd++)
                            {
                                ...
                                if (spd1 != null)
                                {
                                    pld1 = new LG_PLD1()
                                    {
                                        ...
                                        c_iteno = lstRND2[nLoopPd].c_iteno.Trim(),
                                        c_plno = plID,
                                        c_spno = spl.SP,
                                        c_type = "01",
                ...
                if (db != null)
                {
                    if (nChanges > 0)
                    {
                        ...
                        //}
                        //else if (kodePrinsipal.Equals("00159"))
                        //{
